...
class Interpreter
  ...
  def execute_command
    ...
    when 403 then return command_403   # When Cancelar
...
#      when 3   # steps
  ...
  def command_303
    if $Trainer
      $Trainer.name = pbEnterPlayerName(_INTL("Seu nome?"), 1, @parameters[1], $Trainer.name)
      return true
    end
    if $game_actors && $data_actors && $data_actors[@parameters[0]] != nil
      ...
      pbFadeOutIn {
        ...
        $game_actors[@parameters[0]].name = sscreen.pbStartScreen(
           _INTL("Digite o nome de {1}.", $game_actors[@parameters[0]].name),
