...
ItemHandlers::CanUseInBattle.add(:POKEDOLL,proc { |item,pokemon,battler,move,firstAction,battle,scene,showMessages|
  if !battle.wildBattle?
    if showMessages
      scene.pbDisplay(_INTL("As palavras de Oak ecoaram... Há hora e lugar para tudo! Mas não agora."))
    end
    next false
  end
  if !battle.canRun
    scene.pbDisplay(_INTL("Você não pode escapar!")) if showMessages
    next false
  end
  next true
})
...
ItemHandlers::CanUseInBattle.addIf(proc { |item| GameData::Item.get(item).is_poke_ball? },   # Poké Balls
  proc { |item,pokemon,battler,move,firstAction,battle,scene,showMessages|
    if battle.pbPlayer.party_full? && $PokemonStorage.full?
      scene.pbDisplay(_INTL("Não há espaço sobrando no PC!")) if showMessages
      next false
    end
    ...
    #       pbItemUsarsAllActions?.
    if !firstAction
      scene.pbDisplay(_INTL("É impossível mirar sem concentração!")) if showMessages
      next false
    end
    if battler.semiInvulnerable?
      scene.pbDisplay(_INTL("Não adianta! É impossível mirar em um Pokémon que não está à vista!")) if showMessages
      next false
    end

    if $game_switches[SWITCH_SILVERBOSS_BATTLE]
      scene.pbDisplay(_INTL("Não adianta! Ele está agitado demais para mirar!")) if showMessages
      next false
    end
    ...
    if battle.pbOpposingBattlerCount>1 && $game_switches[SWITCH_SILVERBOSS_BATTLE]# !(GameData::Item.get(item).is_snag_ball? && battle.trainerBattle?)
      if battle.pbOpposingBattlerCount==2
        if $game_switches[SWITCH_SILVERBOSS_BATTLE]
          scene.pbDisplay(_INTL("Não adianta! Ele ainda está agitado demais para mirar!")) if showMessages
        else
          scene.pbDisplay(_INTL("Não adianta! É impossível mirar quando há dois Pokémon!")) if showMessages
        end
      else
        if $game_switches[SWITCH_SILVERBOSS_BATTLE]
          scene.pbDisplay(_INTL("Não adianta! Ele ainda está agitado demais para mirar!")) if showMessages
        else
          scene.pbDisplay(_INTL("Não adianta! É impossível mirar quando há mais de um Pokémon!")) if showMessages
        end
      end
      next false
    end
...
ItemHandlers::UseInBattle.add(:GUARDSPEC,proc { |item,battler,battle|
  ...
  battle.pbDisplay(_INTL("{1} ficou envolto em névoa!",battler.pbTeam))
  battler.pokemon.changeHappiness("battleitem")
})

ItemHandlers::UseInBattle.add(:POKEDOLL,proc { |item,battler,battle|
  battle.decision = 3
  battle.pbDisplayPaused(_INTL("Você fugiu em segurança!"))
})
...
ItemHandlers::UseInBattle.add(:POKEFLUTE,proc { |item,battler,battle|
  ...
  battle.pbDisplay(_INTL("Todos os Pokémon se animaram com a melodia!"))
})
...
ItemHandlers::BattleUseOnPokemon.add(:AWAKENING,proc { |item,pokemon,battler,choices,scene|
  ...
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} acordou.",name))
})
...
ItemHandlers::BattleUseOnPokemon.add(:ANTIDOTE,proc { |item,pokemon,battler,choices,scene|
  ...
  name = (battler) ? battler.pbThis : pokemon.name
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} foi curado do envenenamento.",name))
})
...
ItemHandlers::BattleUseOnPokemon.add(:BURNHEAL,proc { |item,pokemon,battler,choices,scene|
  ...
  name = (battler) ? battler.pbThis : pokemon.name
  scene.pbRefresh
  scene.pbDisplay(_INTL("A queimadura de {1} foi curada.",name))
})
...
ItemHandlers::BattleUseOnPokemon.add(:PARALYZEHEAL,proc { |item,pokemon,battler,choices,scene|
  ...
  name = (battler) ? battler.pbThis : pokemon.name
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} foi curado da paralisia.",name))
})
...
ItemHandlers::BattleUseOnPokemon.add(:ICEHEAL,proc { |item,pokemon,battler,choices,scene|
  ...
  name = (battler) ? battler.pbThis : pokemon.name
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} descongelou.",name))
})
...
ItemHandlers::BattleUseOnPokemon.add(:FULLHEAL,proc { |item,pokemon,battler,choices,scene|
  ...
  name = (battler) ? battler.pbThis : pokemon.name
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} ficou saudável.",name))
})
...
ItemHandlers::BattleUseOnPokemon.add(:REVIVE,proc { |item,pokemon,battler,choices,scene|
  ...
  pokemon.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} se recuperou do desmaio!",pokemon.name))
})
...
ItemHandlers::BattleUseOnBattler.add(:REDFLUTE,proc { |item,battler,scene|
  ...
  scene.pbDisplay(_INTL("{1} superou a paixão.",battler.pbThis))
})

ItemHandlers::BattleUseOnBattler.add(:YELLOWFLUTE,proc { |item,battler,scene|
  battler.pbCureConfusion
  scene.pbDisplay(_INTL("{1} saiu da confusão.",battler.pbThis))
})
...
ItemHandlers::BattleUseOnBattler.add(:DIREHIT,proc { |item,battler,scene|
  ...
  scene.pbDisplay(_INTL("{1} está se empolgando!",battler.pbThis))
  battler.pokemon.changeHappiness("battleitem")
})
