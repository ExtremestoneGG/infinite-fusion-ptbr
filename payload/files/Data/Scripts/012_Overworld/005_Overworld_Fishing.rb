...
def pbFishing(hasEncounter,rodType=1)
  ...
  loop do
    ...
    if pbWaitMessage(msgWindow,time)
      ...
      pbMessageDisplay(msgWindow,_INTL("Nem uma fisgada..."))
      break
    end
    if hasEncounter && rand(100)<biteChance
      ...
      if !pbWaitForInput(msgWindow,message+_INTL("\r\nOh! Uma fisgada!"),frames)
        pbFishingEnd
        $game_player.setDefaultCharName(nil,oldpattern)
        pbMessageDisplay(msgWindow,_INTL("O Pokémon escapou..."))
        break
      end
      ...
      if autohook || rand(100) < hookChance
        pbFishingEnd
        pbMessageDisplay(msgWindow,_INTL("Um Pokémon foi fisgado!")) if !autohook
        $game_player.setDefaultCharName(nil,oldpattern)
        ret = true
        break
      end
