...
def pbItemBall(item, quantity = 1, item_name = "", canRandom = true)
  ...
  if $PokemonBag.pbStoreItem(item, quantity) # If item can be picked up
    ...
    if item == :LEFTOVERS
      pbMessage(_INTL("\\me[{1}]Você encontrou alguns \\c[1]{2}\\c[0]!\\wtnp[30]", meName, itemname))
    elsif item.is_machine? # TM or HM
      pbMessage(_INTL("\\me[{1}]Você encontrou \\c[1]{2} {3}\\c[0]!\\wtnp[30]", meName, itemname, GameData::Move.get(move).name))
    elsif quantity > 1
      pbMessage(_INTL("\\me[{1}]Você encontrou {2} {4}{3}\\c[0]!\\wtnp[30]", meName, quantity, itemname, text_color))
    elsif itemname.starts_with_vowel?
      pbMessage(_INTL("\\me[{1}]Você encontrou {3}{2}\\c[0]!\\wtnp[30]", meName, itemname, text_color))
    else
      pbMessage(_INTL("\\me[{1}]Você encontrou {3}{2}\\c[0]!\\wtnp[30]", meName, itemname, text_color))
    end
    pbMessage(_INTL("Você guardou {1}\\nno bolso <icon=bagPocket{2}>\\c[1]{3}\\c[0].",
  ...
  # Can't add the item
  if item == :LEFTOVERS
    pbMessage(_INTL("Você encontrou alguns \\c[1]{1}\\c[0]!\\wtnp[30]", itemname))
  elsif item.is_machine? # TM or HM
    pbMessage(_INTL("Você encontrou \\c[1]{1} {2}\\c[0]!\\wtnp[30]", itemname, GameData::Move.get(move).name))
  elsif quantity > 1
    pbMessage(_INTL("Você encontrou {1} \\c[1]{2}\\c[0]!\\wtnp[30]", quantity, itemname))
  elsif itemname.starts_with_vowel?
    pbMessage(_INTL("Você encontrou \\c[1]{1}\\c[0]!\\wtnp[30]", itemname))
  else
    pbMessage(_INTL("Você encontrou \\c[1]{1}\\c[0]!\\wtnp[30]", itemname))
  end
  pbMessage(_INTL("Mas sua Bag está cheia..."))
  return false
end
...
def pbReceiveItem(item, quantity = 1, item_name = "", music = nil, canRandom = true)
  ...
  if item == :LEFTOVERS || item == :MUSHROOMSPORES
    pbMessage(_INTL("\\me[{1}]Você recebeu alguns \\c[1]{2}\\c[0]!\\wtnp[30]", meName, itemname))
  elsif item.is_machine? # TM or HM
    ...
    # end
    pbMessage(_INTL("\\me[{1}]Você recebeu \\c[1]{2} {3}\\c[0]!\\wtnp[30]", meName, itemname, GameData::Move.get(move).name))
  elsif quantity > 1
    pbMessage(_INTL("\\me[{1}]Você recebeu {2} {4}{3}\\c[0]!\\wtnp[30]", meName, quantity, itemname, text_color))
  elsif itemname.starts_with_vowel?
    pbMessage(_INTL("\\me[{1}]Você recebeu {3}{2}\\c[0]!\\wtnp[30]", meName, itemname, text_color))
  else
    pbMessage(_INTL("\\me[{1}]Você recebeu {3}{2}\\c[0]!\\wtnp[30]", meName, itemname, text_color))
  end
  promptRegisterItem(item)
  if $PokemonBag.pbStoreItem(item, quantity) # If item can be added
    pbMessage(_INTL("Você guardou {1}\\nno bolso <icon=bagPocket{2}>\\c[1]{3}\\c[0].", itemname, pocket, PokemonBag.pocketNames()[pocket]))
    updatePinkanBerryDisplay()
    return true
  end
