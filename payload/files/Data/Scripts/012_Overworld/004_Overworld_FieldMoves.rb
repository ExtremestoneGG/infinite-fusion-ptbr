...
def pbCheckHiddenMoveBadge(badge = -1, showmsg = true)
  ...
  pbMessage(_INTL("Desculpe, uma nova Badge é necessária.")) if showmsg
  return false
end
...
def pbCut
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_CUT, false) || (!$DEBUG && !movefinder)
    if $PokemonBag.pbQuantity(:MACHETE) <= 0
      pbMessage(_INTL("Esta árvore parece que pode ser cortada."))
      return false
    end
  end
  pbMessage(_INTL("Esta árvore parece que pode ser cortada!\1"))
  if pbConfirmMessage(_INTL("Gostaria de usar Cut?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    pbMessage(_INTL("{1} usou {2}!", speciesname, GameData::Move.get(move).name))
...
HiddenMoveHandlers::CanUseMove.add(:CUT, proc { |move, pkmn, showmsg|
  ...
  if !facingEvent || !facingEvent.name[/cuttree/i]
    pbMessage(_INTL("Não dá para usar isso aqui.")) if showmsg
    next false
  end
  next true
})
...
HiddenMoveHandlers::UseMove.add(:CUT, proc { |move, pokemon|
  if !pbHiddenMoveAnimation(pokemon)
    pbMessage(_INTL("{1} usou {2}!", pokemon.name, GameData::Move.get(move).name))
  end
  facingEvent = $game_player.pbFacingEvent
  if facingEvent
...
HiddenMoveHandlers::CanUseMove.add(:DIG, proc { |move, pkmn, showmsg|
  ...
  if $game_player.pbHasDependentEvents?
    pbMessage(_INTL("Não dá para usar isso quando há alguém com você.")) if showmsg
    next false
  end
  next true
})
...
def pbRockClimb
  ...
  if pbConfirmMessage(_INTL("Parece possível escalar aqui. Gostaria de usar Rock Climb?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    pbMessage(_INTL("{1} usou {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    climbLedge
...
def pbDive
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE, false) || (!$DEBUG && !movefinder)
    if $PokemonBag.pbQuantity(:SCUBAGEAR) <= 0
      pbMessage(_INTL("O mar é profundo aqui. Um Pokémon talvez consiga mergulhar."))
      return false
    end
  end
  if pbConfirmMessage(_INTL("O mar é fundo aqui. Gostaria de usar Dive?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    if movefinder
...
def pbSurfacing
  ...
  #   return false
  # end
  if pbConfirmMessage(_INTL("A luz está filtrando de cima. Gostaria de usar Dive?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    pbMessage(_INTL("{1} usou {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbFadeOutIn {
...
HiddenMoveHandlers::CanUseMove.add(:FLASH, proc { |move, pkmn, showmsg|
  ...
  if $PokemonGlobal.flashUsed
    pbMessage(_INTL("Flash já está sendo usado.")) if showmsg
    next false
  end
  next true
})
...
def pbHeadbutt(event = nil)
  ...
  movefinder = $Trainer.get_pokemon_with_move(move)
  if !$DEBUG && !movefinder
    pbMessage(_INTL("Um Pokémon pode estar nesta árvore. Talvez um Pokémon consiga sacudi-la."))
    return false
  end
  if pbConfirmMessage(_INTL("Um Pokémon pode estar nesta árvore. Gostaria de usar Headbutt?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    pbMessage(_INTL("{1} usou {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
...
HiddenMoveHandlers::CanUseMove.add(:RELICSONG, proc { |move, pokemon, showmsg|
  if  !(pokemon.isFusionOf(:MELOETTA_A) || pokemon.isFusionOf(:MELOETTA_P))
    pbMessage(_INTL("Não terá efeito.")) if showmsg
    next false
  end
  next true
})
...
def changeMeloettaForm(pokemon)
  ...
  if !pokemon.isFusion?
    if is_meloetta_A
      ...
      pbMessage(_INTL("{1} mudou para a Pirouette form!", pokemon.name))
    end
    if is_meloetta_P
      changeSpeciesSpecific(pokemon, :MELOETTA_A)
      pbMessage(_INTL("{1} mudou para a Aria form!", pokemon.name))
    end
    return
  end
  if is_meloetta_A && is_meloetta_P
    ...
    pbMessage(_INTL("{1} mudou de forma!", pokemon.name))
  else
    if is_meloetta_P
...
def pbRockSmash
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_ROCKSMASH, false) || (!$DEBUG && !movefinder)
    if $PokemonBag.pbQuantity(:PICKAXE) <= 0
      pbMessage(_INTL("É uma rocha áspera, mas talvez um Pokémon consiga quebrá-la."))
      return false
    end
  end
  if pbConfirmMessage(_INTL("Esta rocha parece quebrável. Gostaria de usar Rock Smash?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    pbMessage(_INTL("{1} usou {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
...
def pbStrength
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_STRENGTH, false) || (!$DEBUG && !movefinder)
    if $PokemonBag.pbQuantity(:LEVER) <= 0
      pbMessage(_INTL("Parece pesado, mas talvez um Pokémon consiga empurrar para o lado."))
      return false
    end
  end
  pbMessage(_INTL("Parece pesado, mas talvez um Pokémon consiga empurrar para o lado.\1"))
  if pbConfirmMessage(_INTL("Gostaria de usar Strength?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    pbMessage(_INTL("{1} usou {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbMessage(_INTL("O Strength de {1} tornou possível mover pedras grandes!", speciesname))
...
HiddenMoveHandlers::CanUseMove.add(:STRENGTH, proc { |move, pkmn, showmsg|
  ...
  if $PokemonMap.strengthUsed
    pbMessage(_INTL("Strength já está sendo usado.")) if showmsg
    next false
  end
  next true
})
...
HiddenMoveHandlers::UseMove.add(:STRENGTH, proc { |move, pokemon|
  if !pbHiddenMoveAnimation(pokemon)
    pbMessage(_INTL("{1} usou {2}!\1", pokemon.name, GameData::Move.get(move).name))
  end
  pbMessage(_INTL("O Strength de {1} tornou possível mover pedras grandes!", pokemon.name))
  $PokemonMap.strengthUsed = true
  next true
})
...
def pbSurf
  ...
  if pbConfirmMessage(_INTL("A água é de um azul profundo...\nGostaria de usar Surf nela?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    pbMessage(_INTL("{1} usou {2}!", speciesname, GameData::Move.get(move).name))
    pbCancelVehicles
    pbHiddenMoveAnimation(movefinder)
...
Events.onAction += proc { |_sender, _e|
  ...
  if $PokemonGlobal.stepcount % 25 == 0
    if !hatUnlocked?(HAT_CARDBOARD_BOX) && rand(2) == 0
      ...
      pbMessage(_INTL("Uau! Um Pokémon pulou para fora da lixeira!"))
      pbWildBattle(:TRUBBISH, 10)
      $PokemonGlobal.stepcount += 1
    end
  else
    if Settings::GAME_ID == :IF_HOENN
      pbMessage(_INTL("Não há nada além de lixo..."))
    end
  end
}
...
HiddenMoveHandlers::CanUseMove.add(:SURF, proc { |move, pkmn, showmsg|
  ...
  if $PokemonGlobal.surfing
    pbMessage(_INTL("Você já está surfando.")) if showmsg
    next false
  end
  if $game_player.pbHasDependentEvents?
    pbMessage(_INTL("Não dá para usar isso quando há alguém com você.")) if showmsg
  ...
  if GameData::MapMetadata.exists?($game_map.map_id) &&
    GameData::MapMetadata.get($game_map.map_id).always_bicycle
    pbMessage(_INTL("Vamos pedalar!")) if showmsg
    next false
  end
  if !$game_player.pbFacingTerrainTag.can_surf_freely ||
    !$game_map.passable?($game_player.x, $game_player.y, $game_player.direction, $game_player)
    pbMessage(_INTL("Nada de surfar aqui!")) if showmsg
    next false
  end
  next true
})
...
def pbSweetScent
  if $game_screen.weather_type != :None
    pbMessage(_INTL("O aroma doce desapareceu por algum motivo..."))
    return
  end
  ...
  if enctype || !$PokemonEncounters.encounter_possible_here? ||
    !pbEncounter(enctype)
    pbMessage(_INTL("Parece não haver nada aqui..."))
  end
end
...
#   if !GameData::MapMetadata.exists?($game_map.map_id) ||
#      !GameData::MapMetadata.get($game_map.map_id).outdoor_map
#     pbMessage("Não dá para usar isso aqui.") if showmsg
#     next false
#   end
...
#   end
#   if $game_player.pbHasDependentEvents?
#     pbMessage("Não dá para usar isso quando há alguém com você.") if showmsg
#     next false
#   end
#   next true
# })
...
#   next false if !healing
#   if !pbHiddenMoveAnimation(pokemon)
#     pbMessage("{1} usou {2}!",pokemon.name,GameData::Move.get(move).name)
#   end
#   pbFadeOutIn {
...
def pbWaterfall
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_WATERFALL, false) || (!$DEBUG && !movefinder)
    if $PokemonBag.pbQuantity(:JETPACK) <= 0
      pbMessage(_INTL("Uma parede de água despenca com um rugido poderoso."))
      return false
    end
  end
  if pbConfirmMessage(_INTL("É uma grande cachoeira. Gostaria de usar Waterfall?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    pbMessage(_INTL("{1} usou {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbAscendWaterfall
...
Events.onAction += proc { |_sender, _e|
  ...
  if terrain.waterfall || isFacingTempWaterfall()
    ...
    pbMessage(_INTL("Uma muralha de água cai com um rugido poderoso."))
  end
}
