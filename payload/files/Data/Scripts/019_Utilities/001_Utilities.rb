...
def pbTrainerName(name = nil, outfit = 0)
  ...
  if name.nil?
    name = pbEnterPlayerName(_INTL("Digite seu nome"), 0, Settings::MAX_PLAYER_NAME_SIZE)
...
def pbMoveTutorChoose(move,movelist=nil,bymachine=false,oneusemachine=false,selectedPokemonVariable=nil)
  ...
  pbFadeOutIn {
    ...
    screen.pbStartScene(_INTL("Ensinar a qual Pokémon?"),false,annot)
    loop do
      ...
      if pokemon.egg?
        pbMessage(_INTL("Eggs não podem aprender golpes.")) { screen.pbUpdate }
      elsif pokemon.shadowPokemon?
        pbMessage(_INTL("Shadow Pokémon não podem aprender golpes.")) { screen.pbUpdate }
      elsif movelist && !movelist.any? { |j| j==pokemon.species }
        pbMessage(_INTL("{1} não pode aprender {2}.",pokemon.name,movename)) { screen.pbUpdate }
