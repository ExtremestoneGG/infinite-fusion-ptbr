...
def pbNickname(pkmn)
  ...
  if pbConfirmMessage(_INTL("Quer dar um apelido a {1}?", species_name))
    pkmn.name = pbEnterPokemonName(_INTL("Apelido de {1}?", species_name),
...
def pbStorePokemon(pkmn)
  if pbBoxesFull?
    pbMessage(_INTL("Não há mais espaço para Pokémon!\1"))
    pbMessage(_INTL("As Boxes de Pokémon estão cheias e não podem aceitar mais nenhum!"))
    return
  end
  ...
  if $Trainer.party_full?
    ...
    if storedbox != oldcurbox
      if creator
        pbMessage(_INTL("A Box \"{1}\" no PC de {2} estava cheia.\1", curboxname, creator))
      else
        pbMessage(_INTL("A Box \"{1}\" no PC estava cheia.\1", curboxname))
      end
      pbMessage(_INTL("{1} foi transferido para a Box \"{2}\".", pkmn.name, boxname))
    else
      if creator
        pbMessage(_INTL("{1} foi transferido para o PC de {2}.\1", pkmn.name, creator))
      else
        pbMessage(_INTL("{1} foi transferido para o PC.\1", pkmn.name))
      end
      pbMessage(_INTL("Foi guardado na Box \"{1}\".", boxname))
    end
  else
...
def pbAddPokemon(pkmn, level = 1, see_form = true, dontRandomize=false, variableToSave=nil)
  ...
  species_name = pkmn.speciesName
  pbMessage(_INTL("{1} recebeu {2}!\\me[Pkmn get]\\wtnp[20]\1", $Trainer.name, species_name))
...
def pbAddToParty(pkmn, level = 1, see_form = true, dontRandomize=false)
  ...
  tryRandomizeGiftPokemon(pkmn,dontRandomize)
  species_name = pkmn.speciesName
  pbMessage(_INTL("{1} recebeu {2}!\\me[Pkmn get]\\wtnp[80]\1", $Trainer.name, species_name))
  pbNicknameAndStore(pkmn)
  $Trainer.pokedex.register(pkmn) if see_form
  return true
end
...
def pbAddForeignPokemon(pkmn, level = 1, owner_name = nil, nickname = nil, owner_gender = 0, see_form = true)
  ...
  pkmn.calc_stats
  if owner_name
    pbMessage(_INTL("\\me[Pkmn get]{1} recebeu um Pokémon de {2}.\1", $Trainer.name, owner_name))
  else
    pbMessage(_INTL("\\me[Pkmn get]{1} recebeu um Pokémon.\1", $Trainer.name))
  end
  pbStorePokemon(pkmn)
  $Trainer.pokedex.register(pkmn) if see_form
  $Trainer.pokedex.set_owned(pkmn.species)
...
def pbGenerateEgg(pkmn, text = "")
  ...
  if $Trainer.party.length<6
    ...
    $PokemonStorage.pbStoreCaught(pkmn)
    Kernel.pbMessage(_INTL("O Egg foi transferido para o PC."))
  end
  return true
end
