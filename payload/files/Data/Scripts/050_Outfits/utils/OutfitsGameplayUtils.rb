...
def obtainClothes(outfit_id)
  ...
  if !outfit
    pbMessage(_INTL("A roupa {1} é inválida.", outfit_id))
    return
  end
  return if !outfit
...
def obtainNewHairstyle(full_outfit_id)
  ...
  pbMessage(_INTL("\\me[{1}]Seu penteado foi alterado para \\c[1]{2}\\c[0]!\\wtnp[30]", musical_effect, hairstyle.name))
  return true
end
