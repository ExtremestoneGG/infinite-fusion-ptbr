class CharacterSelectMenuPresenter
  ...
  MIN_SKIN_COLOR = 1
  MAX_SKIN_COLOR = 6
  SKIN_COLOR_IDS = ["Tipo A", "Tipo B", "Tipo C", "Tipo D", "Tipo E", "Tipo F"]
  GENDERS_IDS = ["Feminino", "Masculino"]

  HAIR_COLOR_IDS = [1, 2, 3, 4]
  HAIR_COLOR_NAMES = ["Loiro", "Castanho claro", "Castanho escuro", "Preto"]
  ...
  def action_button_pressed(current_index)
    ...
    when OPTION_NAME
      ...
      @name = pbEnterPlayerName("Digite seu nome", 0, Settings::MAX_PLAYER_NAME_SIZE)
    ...
    when OPTION_CONFIRM
      ...
      cmd = pbMessage("Estas informações estão corretas?", ["Sim", "Não"])
      if cmd == 0
