class ClothesShopPresenter < PokemonMartScreen
  ...
  def removeDye(item)
    if pbConfirm(_INTL("Tem certeza de que quer remover a tintura de {1}?", item.name))
  ...
  def putOnHats()
    ...
    pbMessage("Você colocou o(s) chapéu(s)!\\wtnp[30]")
  end
  ...
  def playerHatActionsMenu(item)
    ...
    cmd_remove = "Remover chapéu"
    cmd_cancel = "Cancelar"
    cmd_dye = _INTL("Dye Kit")
    cmd_swap = "Trocar posições dos chapéus"

    options = build_options_menu(item,cmd_confirm,cmd_remove,cmd_dye,cmd_swap,cmd_cancel)
    choice = pbMessage("O que gostaria de fazer?", options, -1,nil,0)
