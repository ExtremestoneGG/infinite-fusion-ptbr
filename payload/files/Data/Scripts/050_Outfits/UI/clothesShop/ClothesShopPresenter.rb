class ClothesShopPresenter < PokemonMartScreen
  ...
  def dyeClothes()
    ...
    options = ["Subir", "Descer", "Redefinir", "Confirmar", "Deixa pra lá"]
    previous_input = 0
    ret = false
    while (true)
  ...
  def playerClothesActionsMenu(item)
    ...
    options << "Cancelar"
    choice = pbMessage("O que gostaria de fazer?", options, -1)
  ...
  def quitMenuPrompt()
    ...
    pbPlayCancelSE
    cmd_confirm = "Definir roupa"
    cmd_discard = "Descartar mudanças"
    cmd_cancel = "Cancelar"
    options = [cmd_discard,cmd_confirm,cmd_cancel]
    choice = pbMessage("Você tem mudanças não salvas!",options,3)
    case options[choice]
    when cmd_confirm
    ...
    loop do
      ...
      if !@adapter.isShop?
        if @adapter.is_a?(ClothesMartAdapter)
          ...
          if pbConfirm(_INTL("Gostaria de vestir {1}?", item.name))
      ...
      if !price.is_a?(Integer)
        pbDisplayPaused("Você já tem este item!")
        if pbConfirm(_INTL("Gostaria de vestir {1}?", item.name))
