def genericOutfitsShopMenu(stock = [], itemType = nil, versions = false, isShop=true, message=nil)
  ...
  message = _INTL("Bem-vindo! Como posso ajudar?") if !message
...
SWAP_HAT_POSITIONS_CAPTION = "Trocar posição dos chapéus"
...
def changeOutfit()
  commands = []
  commands[cmdHat = commands.length] = "Trocar chapéu"
  commands[cmdClothes = commands.length] = "Trocar roupa"
  commands[cmdQuit = commands.length] = "Sair"

  cmd = pbMessage("O que gostaria de fazer?", commands, cmdQuit + 1)
  loop do
    if cmd == cmdClothes
      changeClothesMenu()
      break
