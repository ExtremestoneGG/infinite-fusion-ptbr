...
def pbGenerateChallenge(rule, tag)
  oldrule = rule
  yield(_INTL("Preparando para gerar equipes"))
  ...
  iterations.times do |iter|
    ...
    yield(_INTL("Gerando equipes ({1} de {2})", iter + 1, iterations))
    i = 0
...
def pbWriteCup(id, rules)
  ...
  cmd = 0
  if trlists.length != 0
    cmd = pbMessage(_INTL("Gerar equipes de Pokémon para este desafio?"),
       [_INTL("NÃO"), _INTL("SIM, USAR EXISTENTE"), _INTL("SIM, USAR NOVO")], 1)
  else
    cmd = pbMessage(_INTL("Gerar equipes de Pokémon para este desafio?"),
  ...
  if cmd == 1   # Yes, use existing
    cmd = pbMessage(_INTL("Escolha um desafio."), list, -1)
    if cmd >= 0
      pbMessage(_INTL("Este desafio usará a lista de Pokémon de {1}.", list[cmd]))
      for i in 0...trlists.length
        tr = trlists[i]
    ...
    return if !pbConfirmMessage(_INTL("Isso pode demorar bastante. Tem certeza?"))
