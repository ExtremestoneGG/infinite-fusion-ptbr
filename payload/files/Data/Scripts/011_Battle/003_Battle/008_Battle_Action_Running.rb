  ...
  def pbRun(idxBattler,duringBattle=false)
    ...
    if trainerBattle?
      if $DEBUG && Input.press?(Input::CTRL)
        if pbDisplayConfirm(_INTL("Tratar esta batalha como vitória?"))
          @decision = 1
          return 1
        elsif pbDisplayConfirm(_INTL("Tratar esta batalha como derrota?"))
          @decision = 2
          return 1
        end
      elsif @internalBattle
        if pbDisplayConfirm(_INTL("Quer desistir da batalha e sair agora?"))
          pbSEPlay("Battle flee")
          pbDisplay(_INTL("{1} desistiu da batalha!",self.pbPlayer.name))
          @decision = 2
          return 1
        end
      elsif pbDisplayConfirm(_INTL("Quer desistir da batalha e sair agora?"))
        pbSEPlay("Battle flee")
        pbDisplay(_INTL("{1} desistiu da batalha!",self.pbPlayer.name))
        @decision = 3
        return 1
    ...
    if $DEBUG && Input.press?(Input::CTRL)
      pbSEPlay("Battle flee")
      pbDisplayPaused(_INTL("Você escapou em segurança!"))
      @decision = 3
      return 1
    end
    if !@canRun
      pbDisplayPaused(_INTL("Você não pode escapar!"))
      return 0
    end
    if !duringBattle
      ...
      if battler.itemActive?
        if BattleHandlers.triggerRunFromBattleItem(battler.item,battler)
          pbSEPlay("Battle flee")
          pbDisplayPaused(_INTL("{1} fugiu usando {2}!",
      ...
      eachOtherSideBattler(idxBattler) do |b|
        ...
        if BattleHandlers.triggerTrappingTargetAbility(b.ability,battler,b,self)
          pbDisplayPaused(_INTL("{1} impede a fuga com {2}!",b.pbThis,b.abilityName))
          return 0
        end
      end
      eachOtherSideBattler(idxBattler) do |b|
        ...
        if BattleHandlers.triggerTrappingTargetItem(b.item,battler,b,self)
          pbDisplayPaused(_INTL("{1} impede a fuga com {2}!",b.pbThis,b.itemName))
          return 0
        end
      end
    end
    ...
    pbDisplayPaused(_INTL("Você não conseguiu fugir!"))
    return -1
  end
end
