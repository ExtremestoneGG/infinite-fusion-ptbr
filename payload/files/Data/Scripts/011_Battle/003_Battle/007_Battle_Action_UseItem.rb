class PokeBattle_Battle
  ...
  def pbCanUseItemOnPokemon?(item,pkmn,battler,scene,showMessages=true)
    if !pkmn || pkmn.egg?
      scene.pbDisplay(_INTL("Não terá nenhum efeito.")) if showMessages
      return false
    end
    # Embargo
    if battler && battler.effects[PBEffects::Embargo]>0
      scene.pbDisplay(_INTL("O efeito de Embargo impede o uso do item em {1}!",
  ...
  def pbConsumeItemInBag(item,idxBattler)
    ...
    if pbOwnedByPlayer?(idxBattler)
      if !$PokemonBag.pbDeleteItem(item)
        raise _INTL("Tentou consumir um item que não estava na Bag por algum motivo.")
      end
    else
  ...
  def pbReturnUnusedItemToBag(item,idxBattler)
    ...
    if pbOwnedByPlayer?(idxBattler)
      if $PokemonBag && $PokemonBag.pbCanStore?(item)
        ...
        raise _INTL("Não foi possível devolver o item não usado para a Bag por algum motivo.")
      end
    else
      items = pbGetOwnerItems(idxBattler)
  ...
  def pbUseItemMessage(item,trainerName)
    ...
    if itemName.starts_with_vowel?
      pbDisplayBrief(_INTL("{1} usou {2}.",trainerName,itemName))
    else
      pbDisplayBrief(_INTL("{1} usou {2}.",trainerName,itemName))
    end
  end
  ...
  def pbUseItemOnPokemon(item,idxParty,userBattler)
    ...
    pbDisplay(_INTL("Mas não teve efeito!"))
  ...
  def pbUseItemOnBattler(item,idxParty,userBattler)
    ...
    if battler
      ...
      pbDisplay(_INTL("Mas este item não pode ser usado aqui!"))
    end
    # Return unused item to Bag
    pbReturnUnusedItemToBag(item,userBattler.index)
  end
