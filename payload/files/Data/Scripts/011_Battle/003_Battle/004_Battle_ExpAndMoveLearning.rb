class PokeBattle_Battle
  ...
  def pbLearnMove(idxParty, newMove)
    ...
    if pkmn.moves.length < Pokemon::MAX_MOVES
      ...
      pbDisplay(_INTL("{1} aprendeu {2}!", pkmnName, moveName)) { pbSEPlay("Pkmn move learnt") }
      if battler
    ...
    loop do
      pbDisplayPaused(_INTL("{1} quer aprender {2}, mas já conhece {3} golpes.",
                            pkmnName, moveName, pkmn.moves.length.to_word))
      if pbDisplayConfirm(_INTL("Esquecer um golpe para aprender {1}?", moveName))
        pbDisplayPaused(_INTL("Qual golpe deve ser esquecido?"))
        ...
        if forgetMove >= 0
          ...
          pbDisplayPaused(_INTL("1, 2 e... ... ... Pronto!"))
          pbDisplayPaused(_INTL("{1} esqueceu como usar {2}. E...", pkmnName, oldMoveName))
          pbDisplay(_INTL("{1} aprendeu {2}!", pkmnName, moveName)) { pbSEPlay("Pkmn move learnt") }
          battler.pbCheckFormOnMovesetChange if battler
          break
        elsif pbDisplayConfirm(_INTL("Desistir de aprender {1}?", moveName))
          pbDisplay(_INTL("{1} não aprendeu {2}.", pkmnName, moveName))
          break
        end
      elsif pbDisplayConfirm(_INTL("Desistir de aprender {1}?", moveName))
        pbDisplay(_INTL("{1} não aprendeu {2}.", pkmnName, moveName))
