  ...
  def pbCall(idxBattler)
    ...
    if battler.shadowPokemon?
      if battler.inHyperMode?
        ...
        pbDisplay(_INTL("{1} recobrou a consciência com o chamado do Trainer!",battler.pbThis))
      else
        pbDisplay(_INTL("Mas nada aconteceu!"))
      end
...
#    end
  ...
  def pbMegaEvolve(idxBattler)
    ...
    when 1   # Rayquaza
      pbDisplay(_INTL("O desejo fervoroso de {1} alcançou {2}!",trainerName,battler.pbThis))
    else
      pbDisplay(_INTL("{2} de {1} está reagindo a {4} de {3}!",
    ...
    pbDisplay(_INTL("{1} Mega Evoluiu para {2}!",battler.pbThis,megaName))
  ...
  def pbPrimalReversion(idxBattler)
    ...
    pbDisplay(_INTL("Primal Reversion de {1}!\nVoltou à sua forma primal!",battler.pbThis))
  end
end
