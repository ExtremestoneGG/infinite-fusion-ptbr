class PokeBattle_Move
  ...
  def pbDisplayUseMessage(user)
    @battle.pbDisplayBrief(_INTL("{1} usou {2}!",user.pbThis,@name))
  end
    ...
    if target.hasActiveAbility?(:AROMAVEIL)
      if showMessage
        ...
        if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("{1} não foi afetado por causa de {2}!",
    ...
    target.eachAlly do |b|
      ...
      if showMessage
        ...
        if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("{1} não foi afetado por causa de {3} de {2}!",
  ...
  def pbEffectivenessMessage(user,target,numTargets=1)
    ...
    if Effectiveness.super_effective?(target.damageState.typeMod)
      if numTargets>1
        @battle.pbDisplay(_INTL("É super efetivo contra {1}!",target.pbThis(true)))
      else
        @battle.pbDisplay(_INTL("É super efetivo!"))
      end
    elsif Effectiveness.not_very_effective?(target.damageState.typeMod)
      if numTargets>1
        @battle.pbDisplay(_INTL("Não é muito efetivo contra {1}...",target.pbThis(true)))
      else
        @battle.pbDisplay(_INTL("Não é muito efetivo..."))
      end
    end
  end
  ...
  def pbHitEffectivenessMessages(user,target,numTargets=1)
    return if target.damageState.disguise
    if target.damageState.substitute
      @battle.pbDisplay(_INTL("O Substitute recebeu o dano no lugar de {1}!",target.pbThis(true)))
    end
    if target.damageState.critical
      if numTargets>1
        @battle.pbDisplay(_INTL("Um golpe crítico em {1}!",target.pbThis(true)))
      else
        @battle.pbDisplay(_INTL("Um golpe crítico!"))
      end
    end
    ...
    if target.damageState.substitute && target.effects[PBEffects::Substitute]==0
      ...
      @battle.pbDisplay(_INTL("O Substitute de {1} desapareceu!",target.pbThis))
    end
  end
  ...
  def pbEndureKOMessage(target)
    if target.damageState.disguise
      @battle.pbShowAbilitySplash(target)
      if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("Seu disfarce serviu como isca!"))
      else
        @battle.pbDisplay(_INTL("O disfarce de {1} serviu como isca!",target.pbThis))
      end
      @battle.pbHideAbilitySplash(target)
      target.pbChangeForm(1,_INTL("O disfarce de {1} foi quebrado!",target.pbThis))
    elsif target.damageState.endured
      @battle.pbDisplay(_INTL("{1} resistiu ao golpe!",target.pbThis))
    elsif target.damageState.sturdy
      @battle.pbShowAbilitySplash(target)
      if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("{1} resistiu ao golpe!",target.pbThis))
      else
        @battle.pbDisplay(_INTL("{1} aguentou firme com Sturdy!",target.pbThis))
      end
      @battle.pbHideAbilitySplash(target)
    elsif target.damageState.focusSash
      @battle.pbCommonAnimation("UseItem",target)
      @battle.pbDisplay(_INTL("{1} aguentou firme usando Focus Sash!",target.pbThis))
      target.pbConsumeItem
    elsif target.damageState.focusBand
      @battle.pbCommonAnimation("UseItem",target)
      @battle.pbDisplay(_INTL("{1} aguentou firme usando Focus Band!",target.pbThis))
    end
  end
