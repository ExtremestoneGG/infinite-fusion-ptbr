class PokeBattle_Scene
  ...
  def pbCommandMenu(idxBattler,firstAction)
    ...
    cmds = [
       _INTL("O que\n{1} fará?",@battle.battlers[idxBattler].name),
       _INTL("Lutar"),
       _INTL("Bolsa"),
       _INTL("Pokémon"),
       (shadowTrainer) ? _INTL("Chamar") : (firstAction) ? _INTL("Fugir") : _INTL("Cancelar")
    ]
  ...
  def pbPartyScreen(idxBattler,canCancel=false)
    ...
    switchScreen.pbStartScene(_INTL("Escolha um Pokémon."),@battle.pbNumPositions(0,0))
    # Loop while in party screen
    loop do
      # Select a Pokémon
      scene.pbSetHelpText(_INTL("Escolha um Pokémon."))
