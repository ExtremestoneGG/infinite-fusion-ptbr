class PokeBattle_Battler
  ...
  def pbOnAbilityChanged(oldAbil)
    if @effects[PBEffects::Illusion] && oldAbil == :ILLUSION
      ...
      if !@effects[PBEffects::Transform]
        ...
        @battle.pbDisplay(_INTL("{2} de {1} perdeu o efeito!", pbThis, GameData::Ability.get(oldAbil).name))
  ...
  def pbSymbiosis
    ...
    @battle.pbPriority(true).each do |b|
      ...
      if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("{1} compartilhou seu {2} com {3}!",
           b.pbThis,b.itemName,pbThis(true)))
      else
        @battle.pbDisplay(_INTL("{2} de {1} permitiu que compartilhasse seu {3} com {4}!",
  ...
  def pbHeldItemTriggered(item_to_use, own_item = true, fling = false)
    ...
    if hasActiveAbility?(:CHEEKPOUCH) && GameData::Item.get(item_to_use).is_berry? && canHeal?
      ...
      if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("O HP de {1} foi restaurado.", pbThis))
      else
        @battle.pbDisplay(_INTL("{2} de {1} restaurou seu HP.", pbThis, abilityName))
      end
      @battle.pbHideAbilitySplash(self)
    end
