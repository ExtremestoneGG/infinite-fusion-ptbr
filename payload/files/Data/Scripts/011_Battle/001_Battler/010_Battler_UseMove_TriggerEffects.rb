class PokeBattle_Battler
    ...
    if target.opposes?(user)
      ...
      if target.effects[PBEffects::Rage] && !target.fainted?
        if target.pbCanRaiseStatStage?(:ATTACK,target)
          @battle.pbDisplay(_INTL("A raiva de {1} está aumentando!",target.pbThis))
      ...
      if target.effects[PBEffects::Grudge] && target.fainted?
        move.pp = 0
        @battle.pbDisplay(_INTL("{2} de {1} perdeu todo o PP por causa de Grudge!",
  ...
  def pbEffectsAfterMove(user,targets,move,numHits)
    ...
    if user.effects[PBEffects::DestinyBondTarget]>=0 && !user.fainted?
      ...
      @battle.pbDisplay(_INTL("{1} levou o atacante junto!",dbName))
    ...
    if !user.fainted? && !user.effects[PBEffects::Transform] &&
      ...
      if !@battle.pbAllFainted?(user.idxOpposingSide) &&
        ...
        if numFainted>0 && user.form==1
          ...
          @battle.pbDisplay(_INTL("{1} ficou totalmente carregado graças ao seu vínculo com o Trainer!",user.pbThis))
          @battle.pbShowAbilitySplash(user,true)
          @battle.pbHideAbilitySplash(user)
          user.pbChangeForm(2,_INTL("{1} se tornou Ash-Greninja!",user.pbThis))
        end
      end
    end
    # Consume user's Gem
