...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    if @pokemon.hasItem?
      ...
      textpos.push([_INTL("Nenhum"), 16, 346, 0, Color.new(192, 200, 208), Color.new(208, 216, 224)])
    end
  ...
  def drawPageOne
    ...
    textpos = [
      ...
      [_INTL("Espécie"), 238, species_y, 0, base, shadow],
      [@pokemon.speciesName, 435, species_y, 2, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Tipo"), 238, type_y, 0, base, shadow],
    ...
    if @pokemon.isFusion?
      ...
      textpos << [_INTL("Cabeça"), 238, fusion_head_y, 0, base, shadow]
      ...
      textpos << [_INTL("Nº Dex"), 238, fusion_head_y, 0, base, shadow]
    ...
    textpos.push([_INTL("Pontos Exp."), 238, 234, 0, base, shadow])
    textpos.push([@pokemon.exp.to_s_formatted, 488, 266, 1, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    textpos.push([_INTL("Próx. Lv."), 238, 298, 0, base, shadow])
  ...
  def drawPageOneEgg
    ...
    # Write various bits of text
    textpos = [
      [_INTL("REGISTRO"), 26, 10, 0, base, shadow],
      [@pokemon.name, 46, 56, 0, base, shadow],
      [_INTL("Item"), 66, 312, 0, base, shadow]
    ]
    # Write the held item's name
    ...
    if mapname && mapname != ""
      memo += _INTL("<c3=404040,B0B0B0>Um Egg misterioso de Pokémon recebido em <c3=F83820,E09890>{1}<c3=404040,B0B0B0>.\n", mapname)
    else
      memo += _INTL("<c3=404040,B0B0B0>Um Egg misterioso de Pokémon.\n", mapname)
    end
    memo += "\n" # Empty line
    # Write Egg Watch blurb
    memo += _INTL("<c3=404040,B0B0B0>\"Observação do Egg\"\n")
    eggstate = _INTL("Parece que este Egg ainda vai demorar bastante para chocar.")
    eggstate = _INTL("O que vai nascer daqui? Ele não parece perto de chocar.") if @pokemon.steps_to_hatch < 10200
    eggstate = _INTL("Ele parece se mexer de vez em quando. Talvez esteja perto de chocar.") if @pokemon.steps_to_hatch < 2550
    eggstate = _INTL("Dá para ouvir sons vindo de dentro! Ele vai chocar em breve!") if @pokemon.steps_to_hatch < 1275
    memo += sprintf("<c3=404040,B0B0B0>%s\n", eggstate)
    # Draw all text
  ...
  def drawPageTwo
    ...
    mapname = pbGetMapNameFromId(@pokemon.obtain_map)
    mapname = @pokemon.obtain_text if @pokemon.obtain_text && !@pokemon.obtain_text.empty?
    mapname = _INTL("Lugar distante") if nil_or_empty?(mapname)
    memo += sprintf("<c3=F83820,E09890>%s\n", mapname)
    # Write how Pokémon was obtained
    mettext = [_INTL("Encontrado no Lv. {1}.", @pokemon.obtain_level),
               _INTL("Egg recebido."),
               _INTL("Recebido por troca no Lv. {1}.", @pokemon.obtain_level),
               "",
               _INTL("Teve um encontro fatídico no Lv. {1}.", @pokemon.obtain_level)
    ...
    if @pokemon.obtain_method == 1
      ...
      mapname = _INTL("Lugar distante") if nil_or_empty?(mapname)
      memo += sprintf("<c3=F83820,E09890>%s\n", mapname)
      memo += _INTL("<c3=404040,B0B0B0>Egg chocado.\n")
    else
      memo += "\n" # Empty line
    end
    ...
    if showNature
      ...
      characteristics = {
        :HP => [_INTL("Adora comer."),
                _INTL("Tira muitas sonecas."),
                _INTL("Cochila bastante."),
                _INTL("Espalha coisas com frequência."),
                _INTL("Gosta de relaxar.")],
        :ATTACK => [_INTL("Orgulha-se da própria força."),
                    _INTL("Gosta de se debater."),
                    _INTL("É um pouco irritadiço."),
                    _INTL("Gosta de lutar."),
                    _INTL("É irritadiço.")],
        :DEFENSE => [_INTL("Tem corpo robusto."),
                     _INTL("Aguenta golpes."),
                     _INTL("É muito persistente."),
                     _INTL("Tem boa resistência."),
                     _INTL("Tem boa perseverança.")],
        :SPECIAL_ATTACK => [_INTL("É muito curioso."),
                            _INTL("É travesso."),
                            _INTL("É extremamente astuto."),
                            _INTL("Frequentemente se perde em pensamentos."),
                            _INTL("É muito exigente.")],
        :SPECIAL_DEFENSE => [_INTL("Tem força de vontade."),
                             _INTL("É um pouco vaidoso."),
                             _INTL("É muito desafiador."),
                             _INTL("Odeia perder."),
                             _INTL("É um pouco teimoso.")],
        :SPEED => [_INTL("Gosta de correr."),
                   _INTL("É atento a sons."),
                   _INTL("É impetuoso e bobo."),
                   _INTL("É meio brincalhão."),
                   _INTL("Foge rapidamente.")]
      }
  ...
  def pbOptions
    ...
    if !@pokemon.egg?
      if pbBattleChallenge.currentChallenge == -1
        commands[cmdGiveItem = commands.length] = _INTL("Dar item")
        commands[cmdTakeItem = commands.length] = _INTL("Pegar item") if @pokemon.hasItem?
        commands[cmdHat = commands.length] = _INTL("Chapéu") if canPutHatOnPokemon(@pokemon)
      end
      commands[cmdPokedex = commands.length] = _INTL("Ver Pokédex") if $Trainer.has_pokedex
    end
