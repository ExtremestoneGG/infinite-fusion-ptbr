...
class PokemonPokedexMenu_Scene
  ...
  def pbStartScene(commands,commands2)
    ...
    @sprites["headings"]=Window_AdvancedTextPokemon.newWithSize(
       _INTL("<c3=F8F8F8,C02028>VISTOS<r>OBTIDOS</c3>"),286,136,208,64,@viewport)
...
class PokemonPokedexMenuScreen
  ...
  def pbStartScreen
    ...
    $Trainer.pokedex.accessible_dexes.each do |dex|
      if dexnames[dex].nil?
        commands.push(_INTL("Pokédex completa"))
    ...
    commands.push(_INTL("Pokémon obtidos"))

    commands.push(_INTL("Sair"))
