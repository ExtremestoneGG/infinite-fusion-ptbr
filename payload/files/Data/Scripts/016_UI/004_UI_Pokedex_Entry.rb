...
class PokemonPokedexInfo_Scene
  ...
  def drawPageInfo(reloading=false)
    ...
    textpos = [
      ...
      [_INTL("Altura"), 314, 152, 0, base, shadow],
      [_INTL("Peso"), 314, 184, 0, base, shadow]
    ]
    ...
    if species_data.is_fusion
      ...
      if customEntry
        ...
        if $PokemonSystem.use_generated_dex_entries && species_data.is_a?(GameData::FusedSpecies)
          ...
          entryText = _INTL("Nenhuma entrada personalizada da Pokédex está disponível para este Pokémon. Considere enviar uma entrada para este Pokémon no Discord do jogo. Entradas autogeradas da Pokédex podem ser ativadas no menu de opções.")
          shadowColor = shadow
        end
      end
    else
