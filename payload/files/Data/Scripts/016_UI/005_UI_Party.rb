...
class PokemonPartyScreen
  ...
  def pbPokemonGiveScreen(item)
    @scene.pbStartScene(@party, _INTL("Dar a qual Pokémon?"))
    ...
    loop do
      ...
      if cmdEntry >= 0 && command == cmdEntry
        ...
        @scene.pbSummary(pkmnid) {
          @scene.pbSetHelpText((@party.length > 1) ? _INTL("Escolha um Pokémon.") : _INTL("Escolha um Pokémon ou cancele."))
        }
      end
    end
    @scene.pbEndScene
  ...
  def pbChooseAblePokemon(ableProc, allowIneligible = false)
    ...
    @scene.pbStartScene(@party,
                        (@party.length > 1) ? _INTL("Escolha um Pokémon.") : _INTL("Escolha um Pokémon ou cancele."), annot)
    loop do
      @scene.pbSetHelpText(
        (@party.length > 1) ? _INTL("Escolha um Pokémon.") : _INTL("Escolha um Pokémon ou cancele."))
      pkmnid = @scene.pbChoosePokemon
      break if pkmnid < 0
  ...
  def pbPokemonScreen
    @scene.pbStartScene(@party,
                        (@party.length > 1) ? _INTL("Escolha um Pokémon.") : _INTL("Escolha um Pokémon ou cancele."), nil)
    loop do
      @scene.pbSetHelpText((@party.length > 1) ? _INTL("Escolha um Pokémon.") : _INTL("Escolha um Pokémon ou cancele."))
      ...
      if pkmnid.is_a?(Array) && pkmnid[0] == 1 # Switch
        @scene.pbSetHelpText(_INTL("Mover para onde?"))
        oldpkmnid = pkmnid[1]
      ...
      commands[cmdNickname = commands.length] = _INTL("Apelido") if !pkmn.egg?
      commands[cmdLearnMove = commands.length] = _INTL("Relembrar golpes")
      ...
      commands[commands.length] = _INTL("Cancelar")
      command = @scene.pbShowCommands(_INTL("O que fazer com {1}?", pkmn.name), commands)
      havecommand = false
      ...
      if cmdSummary >= 0 && command == cmdSummary
        ...
        command = @scene.pbShowCommands(_INTL("O que fazer com a Mail?"),
                                        [_INTL("Ler"), _INTL("Pegar"), _INTL("Cancelar")])
        case command
        when 0 # Read
          pbFadeOutIn {
        ...
        itemcommands[cmdTakeItem = itemcommands.length] = _INTL("Pegar") if pkmn.hasItem?
        ...
        command = @scene.pbShowCommands(_INTL("O que fazer com um item?"), itemcommands)
...
def pbChoosePokemon(variableNumber, nameVarNumber, ableProc = nil, allowIneligible = false)
  ...
  pbFadeOutIn {
    ...
    if ableProc
      ...
      screen.pbStartScene(_INTL("Escolha um Pokémon."), false)
