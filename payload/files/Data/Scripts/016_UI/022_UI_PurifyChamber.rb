...
class PurifyChamberSet
  ...
  # Câmara de Purificação treats Normal/Normal matchup as super effective
    ...
    for set in 0...NUMSETS
      ...
      if @sets[set].shadow
        if @sets[set].shadow.heart_gauge > 0
          ...
          if isPurifiable?(set)
            pbMessage(_INTL("Seu {1} na Câmara de Purificação está pronto para a purificação!",@sets[set].shadow.name))
          end
        end
      end
    end
...
#    end
  ...
  def pbPlace(pkmn,position)
    ...
    if pkmn.egg?
      @scene.pbDisplay(_INTL("Não dá para colocar um Egg ali."))
      return false
    end
    if (position==0)
      if pkmn.shadowPokemon?
        ...
        @scene.pbDisplay(_INTL("Só um Shadow Pokémon pode ficar ali."))
        return false
      end
    elsif (position>=1)
      if pkmn.shadowPokemon?
        @scene.pbDisplay(_INTL("Não dá para colocar um Shadow Pokémon ali."))
        return false
      else
        oldpkmn=PurifyChamberHelper.pbGetPokemon(@chamber,position)
        if oldpkmn
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set)==0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(
         _INTL("{1} está pronto para abrir seu coração. Porém, é preciso haver pelo menos um Pokémon comum no set para realizar a cerimônia de purificação.",pkmn.name))
    end
  end
  ...
  def pbOpenSetDetail
    ...
    loop do
      ...
      if cmd[0]==0
        ...
        if curpkmn || heldpkmn
          commands=[_INTL("MOVER"),_INTL("RESUMO"),
             _INTL("RETIRAR")]
          if curpkmn && heldpkmn
            commands[0]=_INTL("TROCAR")
          elsif heldpkmn
          ...
          if !heldpkmn && curpkmn && cmd[1]==0 &&
            ...
            commands[cmdRotate=commands.length]=_INTL("GIRAR")
          end
          if !heldpkmn && curpkmn
            commands[cmdReplace=commands.length]=_INTL("SUBSTITUIR")
          end
          commands.push(_INTL("CANCELAR"))
          choice=@scene.pbShowCommands(
             _INTL("O que devo fazer com {1}?",
          ...
          if choice==0
            ...
            if pbBoxesFull?
              @scene.pbDisplay("Todas as Boxes estão cheias.")
            elsif heldpkmn
            ...
            if pos
              ...
              if newpkmn
                if (newpkmn.shadowPokemon?)!=(curpkmn.shadowPokemon?)
                  @scene.pbDisplay(_INTL("Esse Pokémon não pode ser colocado ali."))
                else
        ...
        if heldpkmn
          @scene.pbDisplay("Você está segurando um Pokémon!")
        else
          if !@scene.pbConfirm("Continuar editando os sets?")
            break
          end
        end
      end
    ...
    if pbCheckPurify()
      @scene.pbDisplay(_INTL("Há um Pokémon pronto para abrir seu coração!\1"))
  ...
  def pbDoPurify
    ...
    for i in 0...purifiables.length
      ...
      if (i+1)!=purifiables.length
        @scene.pbDisplay(_INTL("Há outro Pokémon pronto para abrir seu coração!"))
        if !@scene.pbConfirm("Quer trocar de set?")
          @scene.pbCloseSet()
          break
        end
      end
  ...
  def pbStartPurify
    ...
    loop do
      ...
      if set<0
        if !@scene.pbConfirm("Continuar vendo hologramas?")
          break
        end
      else
        chamber.currentSet=set
        cmd=@scene.pbShowCommands(
           _INTL("O que você quer fazer?"),[
           _INTL("EDITAR"),_INTL("TROCAR"),_INTL("CANCELAR")])
        if cmd==0 # edit
          if !pbOpenSetDetail()
            break
          end
...
class PurifyChamberPC
  ...
  def name
    return _INTL("Câmara de Purificação")
  end

  def access
    pbMessage(_INTL("\\se[PC access]Câmara de Purificação acessada."))
    pbPurifyChamber()
  end
end
