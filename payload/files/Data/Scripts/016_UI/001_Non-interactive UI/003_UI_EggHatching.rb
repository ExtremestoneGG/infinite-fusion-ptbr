...
class PokemonEggHatch_Scene
  ...
  def pbMain
    ...
    @pokemon.name = nil
    pbMessage(_INTL("\\se[]{1} chocou do Egg!\\wt[80]", @pokemon.name)) { update }
    if pbConfirmMessage(
        _INTL("Quer dar um apelido ao recém-nascido {1}?", @pokemon.name)) { update }
      nickname = pbEnterPokemonName(_INTL("Apelido de {1}?", @pokemon.name),
    ...
    if !$Trainer.pokedex.owned?(@pokemon.species)
      ...
      pbMessage(_INTL("Os dados de {1} foram adicionados à Pokédex.", @pokemon.name))
...
def pbHatch(pokemon)
  ...
  if !pbHatchAnimation(pokemon)
    ...
    pbMessage(_INTL("...\1"))
    pbMessage(_INTL("... .... .....\1"))
    pbMessage(_INTL("{1} chocou do Egg!", speciesname))
    if pbConfirmMessage(_INTL("Quer dar um apelido ao recém-nascido {1}?", speciesname))
      nickname = pbEnterPokemonName(_INTL("Apelido de {1}?", speciesname),
