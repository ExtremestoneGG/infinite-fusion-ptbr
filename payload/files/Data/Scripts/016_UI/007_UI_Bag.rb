...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def drawItem(index,_count,rect)
    ...
    if index==self.itemCount-1
      textpos.push(["FECHAR BOLSA",rect.x,rect.y-2,false,self.baseColor,self.shadowColor])
    else
...
class PokemonBag_Scene
  ...
  def pbRefreshIndexChanged
    ...
    @sprites["itemtext"].text =
       (itemlist.item) ? GameData::Item.get(itemlist.item).description : "Fechar a bolsa."
  end
...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      commands[cmdRead = commands.length]       = "Ler" if itm.is_mail?
      if ItemHandlers.hasOutHandler(item) || (itm.is_machine? && $Trainer.party.length>0)
        if ItemHandlers.hasUseText(item)
          ...
          commands[cmdUse = commands.length]    = "Usar"
        end
      end
      commands[cmdGive = commands.length]       = "Dar" if $Trainer.pokemon_party.length > 0 && itm.can_hold?
      commands[cmdToss = commands.length]       = "Descartar" if !itm.is_important? || $DEBUG
      if @bag.pbIsRegistered?(item)
        commands[cmdRegister = commands.length] = "Desmarcar"
      elsif pbCanRegisterItem?(item)
        commands[cmdRegister = commands.length] = "Registrar"
      end
      commands[cmdSort = commands.length]        = "Organizar bolsa"
      commands[cmdDebug = commands.length]      = _INTL("Debug") if $DEBUG
      commands[commands.length]                 = "Cancelar"
      # Show commands generated above
      itemname = itm.name
      command = @scene.pbShowCommands(_INTL("{1} foi selecionado.",itemname),commands)
      if cmdRead>=0 && command==cmdRead   # Read mail
        ...
        if $Trainer.pokemon_count == 0
          @scene.pbDisplay("Não há nenhum Pokémon.")
        elsif itm.is_important?
          @scene.pbDisplay(_INTL("{1} não pode ser segurado.",itemname))
        else
          pbFadeOutIn {
        ...
        if qty>1
          helptext = _INTL("Descartar quantos {1}?",itm.name_plural)
          qty = @scene.pbChooseNumber(helptext,qty)
        end
        if qty>0
          itemname = itm.name_plural if qty>1
          if pbConfirm(_INTL("Tudo bem jogar fora {1} {2}?",qty,itemname))
            pbDisplay(_INTL("Jogou fora {1} {2}.",qty,itemname))
        ...
        command = @scene.pbShowCommands("Como organizar?",[
          "Em ordem alfabética",
          "Por quantidade",
          # "Recently used",
          # "Recently obtained",
        ...
        loop do
          ...
          when 0
            ...
            newqty = pbMessageChooseNumber(
               _INTL("Escolha a nova quantidade de {1} (máx. {2}).", itemplural, Settings::BAG_MAX_PER_SLOT),params) { @scene.pbUpdate }
            if newqty>qty
  ...
  def pbWithdrawItemScreen
    ...
    loop do
      ...
      if qty>1 && !itm.is_important?
        qty = @scene.pbChooseNumber("Quantos quer retirar?",qty)
      end
      next if qty<=0
      if @bag.pbCanStore?(item,qty)
        ...
        pbDisplay(_INTL("Retirou {1} {2}.",dispqty,itemname))
      else
        pbDisplay("Não há mais espaço na Bag.")
      end
    end
    @scene.pbEndScene
  end
  ...
  def pbDepositItemScreen
    ...
    loop do
      ...
      qty = @bag.pbQuantity(item)
      if qty>1 && !itm.is_important?
        qty = @scene.pbChooseNumber("Quantos quer depositar?",qty)
      end
      if qty>0
        if !storage.pbCanStore?(item,qty)
          pbDisplay("Não há espaço para guardar itens.")
        else
          ...
          dispqty  = (itm.is_important?) ? 1 : qty
          itemname = (dispqty>1) ? itm.name_plural : itm.name
          pbDisplay(_INTL("Depositou {1} {2}.",dispqty,itemname))
        end
      end
    end
    @scene.pbEndScene
  ...
  def pbTossItemScreen
    ...
    loop do
      ...
      itm = GameData::Item.get(item)
      if itm.is_important?
        @scene.pbDisplay("Isso é importante demais para descartar!")
        next
      end
      qty = storage.pbQuantity(item)
      itemname       = itm.name
      ...
      if qty>1
        qty=@scene.pbChooseNumber(_INTL("Descartar quantos {1}?",itemnameplural),qty)
      end
      if qty>0
