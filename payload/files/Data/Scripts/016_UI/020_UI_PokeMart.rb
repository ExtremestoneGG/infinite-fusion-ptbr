...
class Window_PokemonMart < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    ypos = rect.y
    if index == count - 1
      textpos.push(["CANCELAR", rect.x, ypos - 4, false, self.baseColor, self.shadowColor])
    else
      item = @stock[index]
...
class PokemonMart_Scene
  def initialize(currency_name = "Dinheiro")
  ...
  def pbRefresh
    if @subscene
      ...
      @sprites["itemtextwindow"].text =
        (itemwindow.item) ? @adapter.getDescription(itemwindow.item) : "Sair da loja."
      itemwindow.refresh
    end
  ...
  def pbChooseNumber(helptext, item, maximum)
    ...
    inbagwindow.text = _INTL("Na Bag:<r>{1}  ", qty)
    ...
    loop do
      ...
      if GameData::Item.get(item).is_important?
        ...
        quantity = @scene.pbChooseNumber(
          _INTL("{1}? Certo. Quantos gostaria de comprar?", itemname), item, maxafford)
        next if quantity == 0
        price *= quantity
        if !pbConfirm(_INTL("{1}, e você quer {2}. Vai custar ${3}. OK?",
        ...
        if $PokemonBag
          if quantity >= 10 && GameData::Item.get(item).is_poke_ball? && GameData::Item.exists?(:PREMIERBALL)
            if @adapter.addItem(GameData::Item.get(:PREMIERBALL))
              pbDisplayPaused("Também vou incluir uma Premier Ball.")
            end
          end
        end
      end
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("{1}? Ah, não. Não posso comprar isso.", itemname))
        next
      end
      qty = @adapter.getQuantity(item)
      next if qty == 0
      ...
      if qty > 1
        qty = @scene.pbChooseNumber(
          _INTL("{1}? Quantos gostaria de vender?", itemname), item, qty)
      end
      if qty == 0
        @scene.pbHideMoney
        next
      ...
      price /= 2
      price *= qty
      if pbConfirm(_INTL("Posso pagar ${1}. Tudo bem?", price.to_s_formatted))
        @adapter.setMoney(@adapter.getMoney + price)
        qty.times do
          @adapter.removeItem(item)
        end
        pbDisplayPaused(_INTL("Entregou {1} e recebeu ${2}.", itemname, price.to_s_formatted)) { pbSEPlay("Mart buy item") }
        @scene.pbRefresh
      end
      @scene.pbHideMoney
    end
...
def pbPokemonMart(stock, speech = nil, cantsell = false)
  ...
  cmdSell = -1
  cmdQuit = -1
  commands[cmdBuy = commands.length] = "Comprar"
  commands[cmdSell = commands.length] = "Vender" if !cantsell
  commands[cmdQuit = commands.length] = "Sair"
  cmd = pbMessage(
    speech ? speech : "Bem-vindo! Como posso ajudar?",
    commands, cmdQuit + 1)
  loop do
