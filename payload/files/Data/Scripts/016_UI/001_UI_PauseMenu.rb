...
class PokemonPauseMenu
  ...
  def pbStartPokemonMenu
    ...
    commands[cmdBag = commands.length] = "Bolsa" if !pbInBugContest?
    commands[cmdPokegear = commands.length] = _INTL("Pokégear") if $Trainer.has_pokegear
    commands[cmdTrainer = commands.length] = $Trainer.name
    commands[cmdOutfit = commands.length] = "Roupa" if $Trainer.can_change_outfit
    if pbInSafari?
      if Settings::SAFARI_STEPS <= 0
        @scene.pbShowInfo(_INTL("Balls: {1}", pbSafariState.ballcount))
      else
        @scene.pbShowInfo(_INTL("Passos: {1}/{2}\nBalls: {3}",
      ...
      if pbBugContestState.lastPokemon
        ...
        @scene.pbShowInfo(_INTL("Capturado: Nenhum\nBalls: {1}", pbBugContestState.ballcount))
      end
      commands[cmdQuit = commands.length] = "Sair do Contest"
    else
      commands[cmdSave = commands.length] = "Salvar" if $game_system && !$game_system.save_disabled
    end
    commands[cmdOption = commands.length] = "Opções"
    commands[cmdDebug = commands.length] = _INTL("Debug") if $DEBUG
    commands[cmdEndGame = commands.length] = "Tela inicial"
    loop do
      ...
      if cmdPokedex >= 0 && command == cmdPokedex
        ...
        @scene.pbHideMenu
        if pbInSafari?
          if pbConfirmMessage("Gostaria de sair do Safari Game agora?")
            @scene.pbEndScene
          ...
          if pbConfirmMessage("Gostaria de encerrar o Contest agora?")
            @scene.pbEndScene
        ...
        @scene.pbHideMenu
        if pbConfirmMessage("Tem certeza de que quer sair do jogo e voltar ao menu principal?")
          scene = PokemonSave_Scene.new
          screen = PokemonSaveScreen.new(scene)
          screen.pbSaveScreen
