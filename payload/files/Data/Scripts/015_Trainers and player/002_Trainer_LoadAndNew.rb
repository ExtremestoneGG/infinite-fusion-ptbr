...
def pbLoadTrainer(tr_type, tr_name, tr_version = 0)
  ...
  raise _INTL("O tipo de Trainer {1} não existe.", tr_type) if !tr_type_data
...
def pbNewTrainer(tr_type, tr_name, tr_version, save_changes = true)
  ...
  for i in 0...Settings::MAX_PARTY_SIZE
    if i == 0
      pbMessage(_INTL("Informe o primeiro Pokémon.", i))
    else
      break if !pbConfirmMessage(_INTL("Adicionar outro Pokémon?"))
    end
    loop do
      ...
      if species
        ...
        level = pbMessageChooseNumber(_INTL("Defina o level de {1} (máx. {2}).", GameData::Species.get(species).name, params.maxNumber), params)
...
def pbMissingTrainer(tr_type, tr_name, tr_version)
  ...
  message = ""
  if tr_version != 0
    message = _INTL("Adicionar novo trainer ({1}, {2}, ID {3})?", tr_type.to_s, tr_name, tr_version)
  else
    message = _INTL("Adicionar novo trainer ({1}, {2})?", tr_type.to_s, tr_name)
  end
  cmd = pbMessage(message, [_INTL("Sim"), _INTL("Não")], 2)
