...
class TriadScene
  ...
  def pbChooseTriadCard(cardStorage)
    ...
    @sprites["helpwindow"].text = _INTL("Escolha {1} cartas para usar neste duelo.",@battle.maxCards)
    ...
    loop do
      ...
      if Input.trigger?(Input::USE) || Input.trigger?(Input::BACK)
        ...
        if chosenCards.length==@battle.maxCards
          @sprites["helpwindow"].text = _INTL("{1} cartas foram escolhidas.",@battle.maxCards)
  ...
  def pbViewOpponentCards(numCards)
    @sprites["helpwindow"].text = _INTL("Ver cartas do oponente.")
    choice     = 0
    lastChoice = -1
    loop do
  ...
  def pbPlayerChooseCard(numCards)
    if @battle.openHand
      @sprites["helpwindow"].text = _INTL("Escolha uma carta ou veja o oponente com Z.")
    else
      @sprites["helpwindow"].text = _INTL("Escolha uma carta.")
    end
    choice     = 0
    lastChoice = -1
    loop do
  ...
  def pbPlayerPlaceCard(cardIndex)
    @sprites["helpwindow"].text = _INTL("Coloque a carta.")
    boardX = 0
    boardY = 0
    doRefresh = true
    loop do
...
#         attack += attacker.bonus(defender)
  ...
  def pbStartScreen(opponentName,minLevel,maxLevel,rules=nil,oppdeck=nil,prize=nil)
    ...
    if count<self.maxCards
      @scene.pbDisplayPaused(_INTL("Você não tem cartas suficientes."))
      @scene.pbEndScene
      return 0
    end
    ...
    if oppdeck && oppdeck.is_a?(Array) && oppdeck.length==self.maxCards   # Preset
      ...
      for i in oppdeck
        ...
        if !species_data
          @scene.pbDisplayPaused(_INTL("O oponente tem uma carta inválida, \"{1}\".",i))
          @scene.pbEndScene
          return 0
        end
    ...
    @scene.pbDisplay(_INTL("Escolhendo quem come?a..."))
    @scene.pbUpdateScore
    playerTurn = (rand(2) == 0)
    @scene.pbDisplay(_INTL("{1} come?ar?.", (playerTurn) ? @playerName : @opponentName))
    for i in 0...@width*@height
      position = nil
      triadCard = nil
      cardIndex = 0
      if playerTurn
        ...
        # Opponent's turn
        @scene.pbDisplay(_INTL("{1} está fazendo uma jogada...",@opponentName))
        scores = []
        ...
        if scores.length==0
          @scene.pbDisplay(_INTL("{1} não consegue se mover por algum motivo...",@opponentName))
    ...
    if playerCount==opponentCount
      @scene.pbDisplayPaused(_INTL("O jogo empatou."))
      result = 3
      if @trade==1
        ...
        @scene.pbDisplayPaused(_INTL("Você manteve todas as cartas da sua cor."))
      end
    elsif playerCount>opponentCount
      @scene.pbDisplayPaused(_INTL("{1} venceu contra {2}.",@playerName,@opponentName))
      result = 1
      if prize
        species_data = GameData::Species.try_get(prize)
        if species_data && $PokemonGlobal.triads.pbStoreItem(species_data.id)
          @scene.pbDisplayPaused(_INTL("Recebeu a carta {1} do oponente.", species_data.name))
        end
      else
        case @trade
        when 0   # Gain 1 random card from opponent's deck
          ...
          if $PokemonGlobal.triads.pbStoreItem(card)
            ...
            @scene.pbDisplayPaused(_INTL("Recebeu a carta {1} do oponente.",cardname))
          end
        ...
        when 2   # Gain all opponent's cards
          ...
          @scene.pbDisplayPaused(_INTL("Recebeu todas as cartas do oponente."))
        end
      end
    else
      @scene.pbDisplayPaused(_INTL("{1} perdeu contra {2}.",@playerName,@opponentName))
      result = 2
      case @trade
      when 0   # Lose 1 random card from your deck
        ...
        $PokemonGlobal.triads.pbDeleteItem(card)
        cardname = GameData::Species.get(card).name
        @scene.pbDisplayPaused(_INTL("O oponente ganhou sua carta {1}.",cardname))
      when 1   # Keep only cards of your color
        for card in originalCards
          $PokemonGlobal.triads.pbDeleteItem(card)
        end
        ...
        @scene.pbDisplayPaused(_INTL("Você manteve todas as cartas da sua cor.",cardname))
      when 2   # Lose all your cards
        for card in originalCards
          $PokemonGlobal.triads.pbDeleteItem(card)
        end
        @scene.pbDisplayPaused(_INTL("O oponente ganhou todas as suas cartas."))
      end
    end
    @scene.pbEndScene
    return result
...
def pbBuyTriads
  ...
  if commands.length == 0
    pbMessage(_INTL("Não há cartas que você possa comprar."))
    return
  end
  ...
  loop do
    ...
    if Input.trigger?(Input::BACK)
      ...
      if $Trainer.money<price
        pbMessage(_INTL("Você não tem dinheiro suficiente."))
        next
      end
      ...
      quantity = pbMessageChooseNumber(
         _INTL("A carta {1}? Claro. Quantas você quer?",itemname),params)
      next if quantity<=0
      price *= quantity
      next if !pbConfirmMessage(_INTL("{1}, e você quer {2}. Vai custar ${3}. Tudo bem?",itemname,quantity,price.to_s_formatted))
      if $Trainer.money<price
        pbMessage(_INTL("Você não tem dinheiro suficiente."))
        next
      end
      if !$PokemonGlobal.triads.pbCanStore?(item,quantity)
        pbMessage(_INTL("Você não tem espaço para mais cartas."))
        next
      end
      $PokemonGlobal.triads.pbStoreItem(item,quantity)
      $Trainer.money -= price
      goldwindow.text = _INTL("Money:\r\n{1}",pbGetGoldString)
      pbMessage(_INTL("Aqui está! Obrigado!\\se[Mart buy item]"))
    end
  end
  cmdwindow.dispose
  goldwindow.dispose
...
def pbSellTriads
  ...
  if commands.length==1
    pbMessage(_INTL("Você não tem cartas."))
    return
  end
  # Scroll right before showing screen
  pbScrollMap(4,3,5)
  ...
  while !done
    loop do
      ...
      if Input.trigger?(Input::USE)
        ...
        if price==0
          pbDisplayPaused(_INTL("A carta {1}? Ah, não. Não posso comprar essa.",itemname))
          break
        end
        cmdwindow.active = false
        cmdwindow.update
        if quantity>1
          ...
          params.setCancelValue(0)
          quantity = pbMessageChooseNumber(
             _INTL("A carta {1}? Quantas você quer vender?",itemname),params)
        end
        if quantity>0
          price /= 4
          price *= quantity
          if pbConfirmMessage(_INTL("Posso pagar ${1}. Tudo bem?",price.to_s_formatted))
            $Trainer.money += price
            goldwindow.text = _INTL("Money:\r\n{1}",pbGetGoldString)
