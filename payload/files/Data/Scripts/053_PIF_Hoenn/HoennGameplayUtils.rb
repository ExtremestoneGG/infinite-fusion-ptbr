...
def showTVText(channel = :RANDOM)
  ...
  when :WEATHER
    pbMessage(_INTL("É o canal do tempo! Vamos ver como estão as coisas hoje."))
    pbWeatherMapMap()
  end
end
...
def getTVNewsCaption()
  if $game_switches[SWITCH_REPORTER_AT_PETALBURG]
    return _INTL("Está passando o jornal local. Tem um concurso de cultivo de Berries acontecendo em Petalburg Town!")
  else
    return _INTL("É uma reprise de PokéChef Deluxe. Nada importante no jornal agora.")
  end
end
