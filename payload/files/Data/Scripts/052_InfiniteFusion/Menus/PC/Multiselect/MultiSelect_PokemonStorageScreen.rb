class PokemonStorageScreen
  ...
  def pbBoxCommands
    ...
    cmd_jump = _INTL("Pular")
    cmd_select = _INTL("Selecionar tudo")
  ...
  def multipleSelectedPokemonCommands(selected, pokemonCount)
    commands = []
    cmdMove = _INTL("Mover")
    cmdRelease = _INTL("Soltar")
    cmdCancelar = _INTL("Cancelar")
    cmdSort = _INTL("Ordenar")

    helptext = _INTL("{1} Pokémon selecionado(s).", pokemonCount)

    commands << cmdMove
    commands << cmdSort
    commands << cmdRelease if $DEBUG
    commands << cmdCancelar
    ...
    if box >= 0
      ...
      if need_fill
        ...
        if store_result == :CANT_PLACE || !store_result
          pbDisplay(_INTL("Não há espaço suficiente!"))
          return
        end
  ...
  def pbReleaseMulti(box)
    ...
    for index in selected
      ...
      if pokemon.owner.name == "RENTAL"
        ...
        pbDisplay(_INTL("Remova o mail primeiro."))
        return false
      end
      able_count += 1 if pbAble?(pokemon)
    ...
    command = pbShowCommands(_INTL("Soltar {1} Pokémon?", final_released.length), [_INTL("Não"), _INTL("Sim")])
    if command == 1
      command_confirm = pbShowCommands(_INTL("Os {1} Pokémon serão perdidos para sempre. Soltar mesmo assim?", final_released.length), [_INTL("Não"), _INTL("Sim")])
      if command_confirm == 1
        ...
        @scene.pbRefresh
        pbDisplay(_INTL("Os Pokémon foram soltos."))
        pbDisplay(_INTL("Tchauzinho!"))
        @scene.pbRefresh
      end
    end
    return
