class FusionQuiz
  ...
  def start_quiz(nb_rounds = 3)
    ...
    for i in 1..nb_rounds
      if i == nb_rounds
        pbMessage(_INTL("Prepare-se! A rodada final vem aí!"))
      elsif i == 1
        pbMessage(_INTL("Prepare-se! Lá vem a primeira rodada!"))
      else
        pbMessage(_INTL("Prepare-se! Lá vem a rodada {1}!", i))
      end
      start_quiz_new_round(round_multiplier)

      rounds_left = nb_rounds - i
      if rounds_left > 0
        pbMessage(_INTL("Fim da rodada {1}. Você acumulou {2} pontos até agora.", i, @score))
        prompt_next_round = pbMessage(_INTL("Pronto para ir para a próxima rodada?", i), ["Sim", "Não"])
        if prompt_next_round != 0
          prompt_quit = pbMessage(_INTL("Ainda faltam {1} rodadas. Você só fica com seus pontos se terminar todas as {2} rodadas. Quer mesmo desistir agora?", rounds_left, nb_rounds), ["Sim", "Não"])
          if prompt_quit
            @abandonned = true
            break
          end
        ...
        pbMessage(_INTL("Isso conclui nosso quiz! Você acumulou {1} pontos no total.", @score))
        pbMessage(_INTL("Obrigado por jogar conosco hoje!"))
      end
    end
    end_quiz()
  end
  ...
  def start_quiz_new_round(round_multiplier = 1)
    ...
    #OBSCURED
    correct_answers << new_question(calculate_points_awarded(base_points_q1, round_multiplier), _INTL("Qual Pokémon é o corpo desta fusão?"), @body_id, nil, true, :BODY)
    pbMessage(_INTL("Próxima pergunta!"))
    correct_answers << new_question(calculate_points_awarded(base_points_q2, round_multiplier), _INTL("Qual Pokémon é a cabeça desta fusão?"), @head_id, nil, true, :HEAD)

    #NON-OBSCURED
    if !correct_answers[0] || !correct_answers[1]
      show_fusion_picture(false)
      pbMessage(_INTL("Ok, agora é sua chance de recuperar os pontos que perdeu!"))
      if !correct_answers[0] #1st question redemption
        new_question(calculate_points_awarded(base_points_q1_redemption, round_multiplier), "Qual Pokémon é o corpo desta fusão?", @body_id, @body_choices, false, :BODY)
      ...
      if !correct_answers[1] #2nd question redemption
        new_question(calculate_points_awarded(base_points_q2_redemption, round_multiplier), "Qual Pokémon é a cabeça desta fusão?", @head_id, @head_choices, false, :HEAD)
      end
    else
      pbSEPlay("Applause", 80)
      pbMessage(_INTL("Uau! Uma rodada perfeita! Você ganha mais {1} pontos!", perfect_round_points))
      show_fusion_picture(false, 100)
      pbMessage(_INTL("Vamos ver como era esse Pokémon!"))
    end
  ...
  def refresh_streak_ui()
    ...
    message = _INTL("Sequência: {1}",@current_streak)
    Kernel.pbClearText()
  ...
  def question_answer_followup_dialog(answered_correctly, correct_answer, points_awarded_if_win, other_chance_later = false)
    if !other_chance_later
      pbMessage(_INTL("E a resposta correta era..."))
      pbMessage("...")
    ...
    if answered_correctly
      ...
      increase_streak
      pbMessage(_INTL("Resposta correta!"))
      pbMessage(_INTL("Você ganhou {1} pontos pela resposta. Sua pontuação atual é {2}", points_awarded_if_win, @score.to_s))
    else
      pbSEPlay("buzzer", 80)
      break_streak
      pbMessage(_INTL("Infelizmente, essa resposta estava errada."))
      pbMessage(_INTL("Mas você terá outra chance!")) if other_chance_later
    end
  end
  ...
  def current_streak_dialog()
    ...
    if @current_streak % 4 == 0
      ...
      if @current_streak >= 8
        pbMessage(_INTL("São {1} respostas corretas seguidas. Você está em uma sequência incrível!", @current_streak))
      else
        pbMessage(_INTL("São {1} respostas corretas seguidas. Você está mandando bem!", @current_streak))
      end
      pbMessage(_INTL("Aqui estão {1} pontos extras por manter a sequência!",extra_points))
  ...
  def give_answer(prompt_message, answer_id, choices,question_type=:BODY)
    ...
    while !question_answered
      ...
      confirmed = pbMessage(_INTL("Essa é sua resposta final?"), [_INTL("Sim"), _INTL("Não")])
      if confirmed == 0
