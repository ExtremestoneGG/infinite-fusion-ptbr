...
HiddenMoveHandlers::UseMove.add(:MORNINGSUN,proc{|move,pokemon|
  Kernel.pbMessage(_INTL("{1} usou {2}!",pokemon.name,GameData::Move.get(move).name))
  pbHiddenMoveAnimation(pokemon)
  pbFadeOutIn(99999){
    pbSkipTime(9)
    newTime = pbGetTimeNow.strftime("%I:%M %p")
    Kernel.pbMessage(_INTL("{1} esperou até de manhã...",$Trainer.name))
    Kernel.pbMessage(_INTL("Agora são {1}",newTime))
...
HiddenMoveHandlers::UseMove.add(:MOONLIGHT,proc{|move,pokemon|
  ...
  pbFadeOutIn(99999){
    pbSkipTime(21)
    newTime = pbGetTimeNow.strftime("%I:%M %p")
    Kernel.pbMessage(_INTL("{1} esperou até anoitecer...",$Trainer.name))
    Kernel.pbMessage(_INTL("Agora são {1}",newTime))
    $game_screen.weather(:None,0,0)
    $game_map.refresh
  }
