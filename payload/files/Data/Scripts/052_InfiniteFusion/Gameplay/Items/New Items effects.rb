ItemHandlers::UseInBattle.add(:POKEDOLL, proc { |item, battler, battle|
  battle.decision = 3
  battle.pbDisplayPaused(_INTL("Você escapou em segurança!"))
})
...
ItemHandlers::UseInField.add(:LANTERN, proc { |item|
  Kernel.pbMessage(_INTL("{1} usou a Lantern.", $Trainer.name))
  if useLantern()
    next 1
  else
    next 0
...
def useTeleporter()
  if HiddenMoveHandlers.triggerCanUseMove(:TELEPORT, 0, true)
    Kernel.pbMessage(_INTL("Teleportar para onde?", $Trainer.name))
  ...
  if !$PokemonTemp.flydata
    ...
    Kernel.pbMessage(_INTL("{1} usou o teleporter!", $Trainer.name))
    pbFadeOutIn(99999) {
...
ItemHandlers::UseFromBag.add(:LANTERN, proc { |item|
  ...
  if !darkness || darkness.disposed?
    Kernel.pbMessage(_INTL("A caverna já está iluminada."))
    next false
  end
  Kernel.pbMessage(_INTL("A Lantern iluminou a área!"))
        $PokemonGlobal.flashUsed = true
...
ItemHandlers::UseOnPokemon.add(:TRANSGENDERSTONE, proc { |item, pokemon, scene|
  if pokemon.gender == 0
    pokemon.makeFemale
    scene.pbRefresh
    scene.pbDisplay(_INTL("O Pokémon se tornou fêmea!"))
    next true
  elsif pokemon.gender == 1
    pokemon.makeMale
    scene.pbRefresh
    scene.pbDisplay(_INTL("O Pokémon se tornou macho!"))
...
def changeHiddenAbility1(abilityList, scene, poke)
  ...
  msg = _INTL("Mudar a habilidade de {1} para {2}?", poke.name, PBAbilities.getName(abID1))
  if Kernel.pbConfirmMessage(_INTL(msg))
    poke.setAbility(2)
    abilName1 = PBAbilities.getName(abID1)
    scene.pbDisplay(_INTL("A habilidade de {1} mudou para {2}!", poke.name, PBAbilities.getName(abID1)))
    return true
  else
    return false
  end
...
def changeHiddenAbility2(abilityList, scene, poke)
  return false if !Kernel.pbConfirmMessage(_INTL("Mudar a habilidade de {1}?", poke.name))
  ...
  if (Kernel.pbMessage(_INTL("Escolha uma habilidade."), [_INTL("{1}", abilName2), _INTL("{1}", abilName3)], 2)) == 0
    poke.setAbility(2)
    scene.pbDisplay(_INTL("A habilidade de {1} mudou para {2}!", poke.name, abilName2))
  else
    return false
  end
  poke.setAbility(3)
  scene.pbDisplay(_INTL("A habilidade de {1} mudou para {2}!", poke.name, abilName3))
  return true
end
...
ItemHandlers::UseOnPokemon.add(:INCUBATOR, proc { |item, pokemon, scene|
  if pokemon.egg?
    if pokemon.eggsteps <= 1
      scene.pbDisplay(_INTL("O Egg já está pronto para chocar!"))
      next false
    else
      scene.pbDisplay(_INTL("Incubando..."))
      scene.pbDisplay(_INTL("..."))
      scene.pbDisplay(_INTL("..."))
      scene.pbDisplay(_INTL("Seu Egg está pronto para chocar!"))
      pokemon.eggsteps = 1
      next true
    end
  else
...
ItemHandlers::UseFromBag.add(:DEBUGGER, proc { |item|
  Kernel.pbMessage(_INTL("[{1}]O debugger deve ser usado APENAS se você estiver preso em algum lugar por causa de um glitch.", Settings::GAME_VERSION_NUMBER))
  if Kernel.pbConfirmMessageSerious(_INTL("O uso inadequado deste item pode causar efeitos indesejados e deixar o jogo injogável. Quer continuar?"))
    $game_player.cancelMoveRoute()
    Kernel.pbStartOver(false)
    pbCommonEvent(COMMON_EVENT_FIX_GAME)
    Kernel.pbMessage(_INTL("Reporte o glitch no Discord do jogo, no canal #bug-reports."))
...
def useSleepingBag()
  ...
  choices = [_INTL("1 hora"), _INTL("6 horas"), _INTL("12 horas"), _INTL("24 horas"), _INTL("Cancelar")]
  choice = Kernel.pbMessage(_INTL("Dormir por quanto tempo?"), choices, choices.length)
  echoln choice
  ...
  pbSEPlay("Sleep", 100)
  pbFadeOutIn {
    Kernel.pbMessage(_INTL("{1} dormiu por um tempo...", $Trainer.name))
  }
  time = pbGetTimeNow.strftime("%I:%M %p")
  newDay = getDayOfTheWeek()
  if newDay != oldDay
    Kernel.pbMessage(_INTL("Agora são {1} de {2}.", time, newDay.downcase.capitalize))
  else
    Kernel.pbMessage(_INTL("Agora são {1}.", time))
  end
  return 1
end
...
def useFieldSleepingBag()
  currentSecondsValue = pbGet(UnrealTime::EXTRA_SECONDS)
  confirmed = Kernel.pbConfirmMessage(_INTL("Dormir por uma hora?"))
  if confirmed
    oldDay = getDayOfTheWeek()
    timeAdded = 0
    timeAdded = 3600
...
ItemHandlers::UseFromBag.add(:SLEEPINGBAG, proc { |item|
  ...
  if !mapMetadata || !mapMetadata.outdoor_map
    Kernel.pbMessage(_INTL("Não dá para usar isso aqui..."))
    next 0
  end
  next useSleepingBag()
})
...
ItemHandlers::UseFromBag.add(:MUSHROOMSPORES, proc { |item|
  if $game_switches[SWITCH_SPORES_REPEL]
    if pbQuantity(:MUSHROOMSPORES) >= 2
      if pbConfirmMessage(_INTL("Condensar 2 amostras de esporos em um Repel?"))
      ...
      pbMessage(_INTL("Colete mais uma amostra para condensá-las em um Repel."))
      next 1
    end
  end
  next 0
...
ItemHandlers::UseFromBag.add(:ODDKEYSTONE, proc { |item|
  ...
  if nbSpirits == 107
    Kernel.pbMessage(_INTL("A Odd Keystone parece estar se movendo sozinha."))
    Kernel.pbMessage(_INTL("Vozes podem ser ouvidas sussurrando de dentro dela..."))
    Kernel.pbMessage(_INTL("Só... mais... um..."))
  elsif nbSpirits < TOTAL_SPIRITS_NEEDED
    nbNeeded = TOTAL_SPIRITS_NEEDED - nbSpirits
    Kernel.pbMessage(_INTL("Vozes podem ser ouvidas sussurrando da Odd Keystone..."))
    Kernel.pbMessage(_INTL("Traga... para nós... {1}... espíritos", nbNeeded.to_s))
  else
    Kernel.pbMessage(_INTL("A Odd Keystone parece estar se movendo sozinha."))
    Kernel.pbMessage(_INTL("Parece que alguma energia poderosa está tentando escapar dela."))
    if (Kernel.pbMessage(_INTL("Soltar?"), [_INTL("Não"), _INTL("Sim")], 0)) == 1
...
def useFavoriteOutfit()
  cmd_switch = isWearingFavoriteOutfit() ? _INTL("Tirar outfit favorito") : _INTL("Vestir outfit favorito")
  cmd_mark_favorite = _INTL("Marcar outfit atual como favorito")
  ...
  if options[choice] == cmd_switch
    ...
    pbMessage(_INTL("Seu outfit favorito foi atualizado!"))
  end
end

def switchToFavoriteOutfit()
  if !$Trainer.favorite_clothes && !$Trainer.favorite_hat && !$Trainer.favorite_hat2
    pbMessage(_INTL("Você pode marcar roupas e chapéus como favoritos no menu de outfits e usar isto para trocar rapidamente!"))
    return 0
  end

  if isWearingFavoriteOutfit()
    if (Kernel.pbConfirmMessage(_INTL("Remover seu outfit favorito?")))
    ...
    if (Kernel.pbConfirmMessage(_INTL("Vestir seu outfit favorito?")))
...
def useRocketUniform()
  ...
  if isWearingTeamRocketOutfit()
    if (Kernel.pbConfirmMessage(_INTL("Tirar o uniforme da Team Rocket?")))
    ...
    if (Kernel.pbConfirmMessage(_INTL("Vestir o uniforme da Team Rocket?")))
      playOutfitChangeAnimation()
...
def useDreamMirror
  ...
  map_name = visitedMap ? Kernel.getMapName(pbGet(226)).to_s : _INTL("um local desconhecido")

  Kernel.pbMessage(_INTL("Você espiou no Dream Mirror..."))

  Kernel.pbMessage(_INTL("Você vê um vislumbre fraco de {1} no reflexo.", map_name))
end

def useStrangePlant
  if darknessEffectOnCurrentMap()
    Kernel.pbMessage(_INTL("A planta estranha parece estar brilhando."))
    $scene.spriteset.addUsarrSprite(LightEffect_GlowPlant.new($game_player))
  else
    Kernel.pbMessage(_INTL("Não teve efeito"))
  end
...
ItemHandlers::UseFromBag.add(:MAGICBOOTS, proc { |item|
  if $DEBUG
    if Kernel.pbConfirmMessageSerious(_INTL("Tirar as Magic Boots?"))
      $DEBUG = false
    end
  else
    if Kernel.pbConfirmMessageSerious(_INTL("Colocar as Magic Boots?"))
      Kernel.pbMessage(_INTL("O modo Debug agora está ativo."))
...
def useSplicerFromField(item)
  ...
  scene.pbStartScene($Trainer.party, _INTL("Escolha um Pokémon"))
  screen = PokemonPartyScreen.new(scene, $Trainer.party)
  chosen = screen.pbChoosePokemon(_INTL("Escolha um Pokémon"))
...
def reverseFusion(pokemon)
  if pokemon.owner.name  == "RENTAL"
    pbMessage(_INTL("Você não pode inverter um Pokémon alugado!"))
    return
  end
...
#         scene.pbDisplay("Sua equipe está cheia! Você não pode desfundir {1}.", pokemon.name)
#         return false
#       else
...
#         pokemon.form = 0
#         scene.pbHardRefresh
#         scene.pbDisplay("{1} mudou de forma!", pokemon.name)
#         return true
#       end
#     else
#       chosen = scene.pbChoosePokemon("Fundir com qual Pokémon?")
#       if chosen >= 0
#         poke2 = $Trainer.party[chosen]
#         if (poke2.species <= NB_POKEMON) && poke2 != pokemon
#           #check if fainted
#           if pokemon.hp == 0 || poke2.hp == 0
#             scene.pbDisplay("Um Pokémon desmaiado não pode ser fundido!")
#             return false
#           end
...
#           end
#         elsif pokemon == poke2
#           scene.pbDisplay("{1} não pode ser fundido consigo mesmo!", pokemon.name)
#           return false
#         else
#           scene.pbDisplay("{1} não pode ser fundido com {2}.", poke2.name, pokemon.name)
#           return false
#
#         end
#
...
#     scene.pbDisplay("Você não pode desfundir um Pokémon recebido em troca!")
#     return false
#   else
#     if Kernel.pbConfirmMessageSerious("{1} deve ser desfundido?", pokemon.name)
#       if pokemon.species > (NB_POKEMON * NB_POKEMON) + NB_POKEMON #triple fusion
#         scene.pbDisplay("{1} não pode ser desfundido.", pokemon.name)
#         return false
#       elsif $Trainer.party.length >= 6 && !pcPosition
#         scene.pbDisplay("Sua equipe está cheia! Você não pode desfundir {1}.", pokemon.name)
#         return false
#       else
#         scene.pbDisplay("Desfundindo... ")
#         scene.pbDisplay(" ... ")
#         scene.pbDisplay(" ... ")
#
...
#         scene.pbHardRefresh
#         scene.pbDisplay("Seus Pokémon foram desfundidos com sucesso! ")
#         return true
#       end
#     end
#   end
...
ItemHandlers::UseOnPokemon.add(:DAMAGEUP, proc { |item, pokemon, scene|
  move = scene.pbChooseMove(pokemon, _INTL("Aumentar o dano de qual golpe?"))
  if move >= 0
    # if pokemon.moves[move].damage==0 ||  pokemon.moves[move].accuracy<=5 || pokemon.moves[move].dmgup >=3
    #  scene.pbDisplay("Isso não terá efeito.")
    #  next false
    # else
    ...
    # next true
    scene.pbDisplay(_INTL("Este item ainda não foi implementado no jogo. Não teve efeito."))
    next false
    # end
  end
})
...
def evolveSlowpokeTail(item, pokemon, scene)
  if pokemon.species != :SHELLDER
    pbMessage(_INTL("Isso não terá efeito."))
    return false
  end
  pbFadeOutInWithMusic(99999) {
    evo = PokemonEvolutionScene.new
...
ItemHandlers::UseOnPokemon.add(:POISONMUSHROOM, proc { |item, pkmn, scene|
  if pkmn.status != :POISON
    pkmn.status = :POISON
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1} foi envenenado ao comer o cogumelo.", pkmn.name))
  end
...
ItemHandlers::BattleUseOnPokemon.add(:POISONMUSHROOM, proc { |item, pokemon, battler, choices, scene|
  if pokemon.status != :POISON
    ...
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1} foi envenenado ao comer o cogumelo.", pokemon.name))
  end
...
#     Kernel.pbMessage("Não roaming Pokémon defined.")
#   else
#     text = "\\l[8]"
...
ItemHandlers::UseFromBag.add(:EXPALL, proc { |item|
  ...
  Kernel.pbMessage(_INTL("O Exp All foi desligado."))
...
ItemHandlers::UseFromBag.add(:EXPALLOFF, proc { |item|
  ...
  Kernel.pbMessage(_INTL("O Exp All foi ligado."))
...
ItemHandlers::UseOnPokemon.add(:INCUBATOR_NORMAL, proc { |item, pokemon, scene|
  if pokemon.egg?
    ...
    # steps -= 2000 / (pokemon.nbIncubatorsUsard + 1).ceil
    if steps <= 1
      pokemon.steps_to_hatch = 1
    else
    ...
    scene.pbDisplay(_INTL("..."))
    scene.pbDisplay(_INTL("..."))
    scene.pbDisplay(_INTL("O Egg está mais perto de chocar!"))

    # if pokemon.steps_to_hatch <= 1
    #   scene.pbDisplay("Incubando...")
    #   scene.pbDisplay("...")
    #   scene.pbDisplay("...")
    ...
    #   scene.pbDisplay("...")
    #   scene.pbDisplay("...")
    #   if pokemon.nbIncubatorsUsard >= 10
...
def pbDNASplicing(pokemon, scene, item = :DNASPLICERS)
  ...
  if (pokemon.species_data.id_number <= NB_POKEMON)
    if pokemon.fused != nil
      if $Trainer.party.length >= 6
        scene.pbDisplay(_INTL("Sua equipe está cheia! Você não pode desfundir {1}.", pokemon.name))
        return false
      else
        $Trainer.party[$Trainer.party.length] = pokemon.fused
        pokemon.fused = nil
        pokemon.form = 0
        scene.pbHardRefresh
        scene.pbDisplay(_INTL("{1} mudou de forma!", pokemon.name))
        return true
      end
    else
      chosen = scene.pbChoosePokemon(_INTL("Fundir com qual Pokémon?"))
      if chosen >= 0
        poke2 = $Trainer.party[chosen]
        if (poke2.species_data.id_number <= NB_POKEMON) && poke2 != pokemon
          ...
          if pokemon.egg? || poke2.egg?
            scene.pbDisplay(_INTL("É impossível fundir um Egg!"))
            return false
          end
          if pokemon.hp == 0 || poke2.hp == 0
            scene.pbDisplay(_INTL("Um Pokémon desmaiado não pode ser fundido!"))
            return false
          end
          ...
          if !firstOptionSelected
            ...
            if chosen == -1
              scene.pbDisplay(_INTL("Ocorreu um erro..."))
              return false
            end
          end

          if (Kernel.pbConfirmMessage(_INTL("Fundir {1} e {2}?", selectedHead.name, selectedBase.name)))
          ...
          scene.pbDisplay(_INTL("{1} não pode ser fundido consigo mesmo!", pokemon.name))
          return false
        else
          scene.pbDisplay(_INTL("{1} não pode ser fundido com {2}.", poke2.name, pokemon.name))
          return false
        end
      else
        return false
...
def pbUnfuse(pokemon, scene, supersplicers, pcPosition = nil)
  if pokemon.species_data.id_number > (NB_POKEMON * NB_POKEMON) + NB_POKEMON # triple fusion
    scene.pbDisplay(_INTL("{1} não pode ser desfundido.", pokemon.name))
    return false
  end
  if pokemon.owner.name  == "RENTAL"
    scene.pbDisplay(_INTL("Você não pode desfundir um Pokémon alugado!"))
    return
  end
  ...
  if (pokemon.foreign?($Trainer)) # && !canunfuse
    scene.pbDisplay(_INTL("Você não pode desfundir um Pokémon recebido em troca!"))
    return false
  else
    if Kernel.pbConfirmMessageSerious(_INTL("{1} deve ser desfundido?", pokemon.name))
      keepInParty = 0
      if $Trainer.party.length >= 6 && !pcPosition

        message = _INTL("Sua equipe está cheia! Manter qual Pokémon na equipe?")
        message = _INTL("Sua equipe está cheia! Manter qual Pokémon na equipe? O outro será solto.") if isOnPinkanIsland()
        scene.pbDisplay(_INTL(message))
        selectPokemonMessage = _INTL("Escolha um Pokémon para manter na sua equipe.")
        selectPokemonMessage = _INTL("Escolha um Pokémon para manter na sua equipe. O outro será solto.") if isOnPinkanIsland()
        choice = Kernel.pbMessage(selectPokemonMessage, ["#{PBSpecies.getName(bodyPoke)}", "#{PBSpecies.getName(headPoke)}", _INTL("Cancelar")], 2)
        if choice == 2
          return false
        else
          keepInParty = choice
      ...
      scene.pbDisplay(_INTL("Desfundindo... "))
      scene.pbDisplay(_INTL(" ... "))
      scene.pbDisplay(_INTL(" ... "))
      ...
      if $Trainer.party.length >= 6
        if (keepInParty == 0)
          if isOnPinkanIsland()
            scene.pbDisplay(_INTL("{1} foi solto.", poke2.name))
          else
            $PokemonStorage.pbStoreCaught(poke2)
            scene.pbDisplay(_INTL("{1} foi enviado ao PC.", poke2.name))
          end
        else
      ...
      # scene.pbDisplay(p1.to_s + " " + p2.to_s)
      scene.pbHardRefresh
      scene.pbDisplay(_INTL("Seus Pokémon foram desfundidos com sucesso!"))
      return true
    end
  end
end
...
def track_pokemon()
  ...
  currently_roaming.each do |roamer_id|
    ...
    case weather_type
    when :Storm
      forecast_msg = _INTL("Uma \\c[6]tempestade de raios\\c[0] incomum foi detectada perto de \\c[6]{1}", map_name)
    when :StrongWinds
      forecast_msg = _INTL("\\c[9]Ventos fortes\\c[0] incomuns foram detectados perto de \\c[9]{1}", map_name)
    when :Sunny
      forecast_msg = _INTL("\\c[10]Luz solar intensa\\c[0] incomum foi detectada perto de \\c[10]{1}", map_name)
    end
    weather_data << forecast_msg if forecast_msg && !weather_data.include?(forecast_msg)
  end
  weather_data << _INTL("Nenhum padrão climático incomum foi detectado.") if weather_data.empty?
  ...
  #   end
  #   if nbRoaming == 0
  #     text = "Não Pokémon appears to be roaming at this moment."
  #   end
  #   Kernel.pbMessage(text)
  # end
end
...
ItemHandlers::UseInField.add(:BOXLINK, proc { |item|
  ...
  if blacklisted_maps.include?($game_map.map_id)
    Kernel.pbMessage(_INTL("Parece não haver cobertura de rede aqui..."))
  else
    pbFadeOutIn {
...
def changeOricorioFormFromItem(pokemon,form_name,new_form)
  if !(Kernel.isPartPokemon(pokemon, :ORICORIO_1) ||
    ...
    scene.pbDisplay(_INTL("Não teve efeito."))
    return false
  end
  if changeOricorioForm(pokemon, new_form)
    pbMessage(_INTL("{1} mudou para o estilo {2}", pokemon.name, form_name))
    pbSet(1, pokemon.name)
    return true
  else
    pbMessage(_INTL("{1} permaneceu igual...", pokemon.name, form_name))
    return false
  end
end
