...
##=## * The "Concluída" field can be pre-set, but is normally only changed in-game
##=##===========================================================================
class Quest
  attr_accessor :id
  attr_accessor :name
...
QuestBranchHotels = _INTL("Quests de hotel")
QuestBranchField = _INTL("Quests de campo")
QuestBranchRocket = _INTL("Quests da Team Rocket")
QuestBranchLegendary = _INTL("Quests lendárias")
...
def finishQuest(id, silent=false)
  ...
  Kernel.pbMessage(_INTL("\\qp\\C[6]Missão concluída!")) if !silent
...
class Questlog
  def initialize
    ...
    pbDrawOutlineText(@main, 0, 2 - 178, 512, 384, "Diário de quests", Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
    ...
    #pbDrawOutlineText(@main, 0, 142 - 178, 512, 384, "Ativas: " + @ongoing.size.to_s, Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
    #pbDrawOutlineText(@main, 0, 198 - 178, 512, 384, "Concluídas: " + @completed.size.to_s, Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
    pbDrawOutlineText(@main, 0, 142, 512, 384, _INTL("Ativas: ") + @ongoing.size.to_s, Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
    pbDrawOutlineText(@main, 0, 198, 512, 384, _INTL("Concluídas: ") + @completed.size.to_s, Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
  ...
  def pbLoad(page)
    ...
    if page == 0
      ...
      if !quest.completed
        pbDrawOutlineText(@main, 8, 250, 512, 384, _INTL("Pendente"), pbColor(:LIGHTRED), Color.new(0, 0, 0))
      else
        pbDrawOutlineText(@main, 8, 250, 512, 384, _INTL("Concluída"), pbColor(:LIGHTBLUE), Color.new(0, 0, 0))
      end
      10.times do |i|
        Graphics.update
        @sprites["main"].opacity += 32
      ...
      pbDrawOutlineText(@text2, 188, -122, 512, 384, _INTL("Recebida em:"), Color.new(255, 255, 255), Color.new(0, 0, 0))
      pbDrawOutlineText(@text2, 188, -94, 512, 384, quest.location, Color.new(255, 172, 115), Color.new(0, 0, 0))
      pbDrawOutlineText(@text2, 188, -62, 512, 384, _INTL("Recebida às:"), Color.new(255, 255, 255), Color.new(0, 0, 0))
      ...
      pbDrawOutlineText(@text2, 188, -4, 512, 384, _INTL("Recebida de:"), Color.new(255, 255, 255), Color.new(0, 0, 0))
      pbDrawOutlineText(@text2, 188, 22, 512, 384, quest.npc, Color.new(255, 172, 115), Color.new(0, 0, 0))
      pbDrawOutlineText(@text2, 188, 162, 512, 384, _INTL("De ") + quest.npc, Color.new(255, 172, 115), Color.new(0, 0, 0))
      pbDrawOutlineText(@text2, 10, -178, 512, 384, quest.name, quest.color, Color.new(0, 0, 0))
      if !quest.completed
        pbDrawOutlineText(@text2, 8, 136, 512, 384, _INTL("Pendente"), pbColor(:LIGHTRED), Color.new(0, 0, 0))
      else
        pbDrawOutlineText(@text2, 8, 136, 512, 384, _INTL("Concluída"), pbColor(:LIGHTBLUE), Color.new(0, 0, 0))
      end
      @sprites["text2"].x = 512
      16.times do
        Graphics.update
      ...
      pbDrawOutlineText(@text, 188, 162, 512, 384, _INTL("De {1}",quest.npc), Color.new(255, 172, 115), Color.new(0, 0, 0))
      pbDrawOutlineText(@text, 10, -178, 512, 384, quest.name, quest.color, Color.new(0, 0, 0))
      if !quest.completed
        pbDrawOutlineText(@text, 8, 136, 512, 384, _INTL("Pendente"), pbColor(:LIGHTRED), Color.new(0, 0, 0))
      else
        pbDrawOutlineText(@text, 8, 136, 512, 384, _INTL("Concluída"), pbColor(:LIGHTBLUE), Color.new(0, 0, 0))
      end
      @sprites["text"].x = -512
      16.times do
        Graphics.update
  ...
  def pbMain
    ...
    @sel_two = 0
    @scene = 0
    pbDrawOutlineText(@main, 0, 2, 512, 384, _INTL("Diário de quests"), Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
    pbDrawOutlineText(@main, 0, 142, 512, 384, _INTL("Ativas: ") + @ongoing.size.to_s, Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
    pbDrawOutlineText(@main, 0, 198, 512, 384, _INTL("Concluídas: ") + @completed.size.to_s, Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
    12.times do |i|
      Graphics.update
    ...
    if dir == :DOWN
      ...
      if @box == 5
        ...
        if @mode == 0
          ...
          pbDrawOutlineText(@main, 0, 2, 512, 384, _INTL("Quests ativas"), Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
        else
          for i in 0...@completed.size
            break if i > 5
            j = (i == 0 ? -5 : (i == 1 ? -4 : (i == 2 ? -3 : (i == 3 ? -2 : (i == 4 ? -1 : 0)))))
          ...
          pbDrawOutlineText(@main, 0, 2 - 178, 512, 384, _INTL("Quests concluídas"), Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
        end
      end
    else
      return if @sel_two == 0
  ...
  def pbList(id)
    ...
    if id == 0
      ...
      pbDrawOutlineText(@main, 0, 175, 512, 384, _INTL("Nenhuma missão ativa"), pbColor(:WHITE), pbColor(:BLACK), 1) if @ongoing.size == 0
      pbDrawOutlineText(@main, 0, 2, 512, 384, _INTL("Quests ativas"), Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
      12.times do |i|
        Graphics.update
      ...
      pbDrawOutlineText(@main, 0, 175, 512, 384, _INTL("Nenhuma missão concluída"), pbColor(:WHITE), pbColor(:BLACK), 1) if @completed.size == 0
      pbDrawOutlineText(@main, 0, 2, 512, 384, _INTL("Quests concluídas"), Color.new(255, 255, 255), Color.new(0, 0, 0), 1)
      12.times do |i|
        Graphics.update
        @sprites["main"].opacity += 32 if i < 8
...
def fix_quest_ids
  ...
  $Trainer.quests.each do |quest|
    ...
    if new_id != quest.id
      ...
      echoln "Concluídas: #{quest.completed}"
      echoln ""
...
def showQuestStatistics(eventId,includeRocketQuests=false)
  ...
  pbCallBub(2, eventId)
  pbMessage(_INTL("Quests aceitas: \\C[1]{1}",quests_accepted.length))
  pbCallBub(2, eventId)
  pbMessage(_INTL("Quests concluídas: \\C[1]{1}",quests_completed.length))
  pbCallBub(2, eventId)
  pbMessage(_INTL("Em andamento: \\C[1]{1}",quests_in_progress.length))
end
...
def getQuestReward(eventId)
  ...
  for reward in rewards_to_give
    ...
    pbCallBub(2, eventId)
    pbMessage(_INTL("Ah, e tem mais uma coisa..."))
    pbCallBub(2, eventId)
    pbMessage(_INTL("Como presente por ter ajudado tanta gente, quero te dar isto."))
  ...
  if nb_to_next_reward <= 0
    pbMessage(_INTL("Não tenho mais recompensas para te dar! Obrigado por ajudar tanta gente!"))
  elsif nb_to_next_reward == 1
    pbMessage(_INTL("Ajude mais {1} pessoa e eu te darei algo bom!",nb_to_next_reward))
  else
    pbMessage(_INTL("Ajude mais {1} pessoas e eu te darei algo bom!",nb_to_next_reward))
  end
end
