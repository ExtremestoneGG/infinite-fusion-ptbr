...
def finishTRQuest(id, status, silent = false)
  ...
  Kernel.pbMessage("\\C[2]Missão concluída!") if status == :SUCCESS && !silent
  Kernel.pbMessage("\\C[2]Missão falhou...") if status == :FAILURE && !silent
...
TR_QUESTS = {
  "tr_cerulean_1" => Quest.new("tr_cerulean_1", _INTL("Insetos rastejantes"), _INTL("O Capitão da Team Rocket encarregou você de acabar com a infestação de insetos no Rocket HQ temporário em Cerulean City."),  "rocket_petrel", _INTL("Cerulean City"), TRQuestColor),
  "tr_cerulean_2" => Quest.new("tr_cerulean_2", _INTL("Zona sem pesca"), _INTL("Intimide os pescadores na Nugget Bridge até eles irem embora da área."),  "rocket_petrel", _INTL("Cerulean City"), TRQuestColor),
  "tr_cerulean_3" => Quest.new("tr_cerulean_3", _INTL("Pokémon desobediente"), _INTL("Traga de volta desmaiado o Pokémon dado pelo Capitão da Team Rocket para ensinar uma lição a ele."),  "rocket_petrel", _INTL("Cerulean City"), TRQuestColor),
  "tr_cerulean_4" => Quest.new("tr_cerulean_4", _INTL("Gran Theft Pokémon!"), _INTL("Siga Petrel e roube um Pokémon raro de uma garotinha."),  "rocket_petrel", _INTL("Cerulean City"), TRQuestColor),

  "tr_celadon_1" => Quest.new("tr_celadon_1", _INTL("Abastecendo os novos grunts"), _INTL("Capture 4 Pokémon com Rocket Balls nos arredores de Celadon City."),  "rocket_archer", _INTL("Celadon City"), TRQuestColor),
  "tr_celadon_2" => Quest.new("tr_celadon_2", _INTL("Interceptação!"), _INTL("Intercepte o carregamento de TMs para a Celadon Store e finja ser a pessoa da entrega para entregar TMs falsas."),  "rocket_archer", _INTL("Celadon City"), TRQuestColor),
  "tr_celadon_3" => Quest.new( "tr_celadon_3", _INTL("Colecionador de Pokémon"), _INTL("Encontre um colecionador de Pokémon na Route 22, perto de Viridian City, e pegue o Pokémon raro dele."),  "rocket_archer", _INTL("Celadon City"), TRQuestColor),
  "tr_celadon_4" => Quest.new("tr_celadon_4", _INTL("Operação Shutdown"), _INTL("O Team Rocket HQ está sendo invadido! Reagrupe-se com o resto dos grunts em Goldenrod Tunnel!"),  "rocket_archer", _INTL("Goldenrod City"), TRQuestColor),

  "tr_pinkan" => Quest.new("tr_pinkan", _INTL("Pinkan Island!"), _INTL("Ajude a Team Rocket em um assalto a uma reserva natural de Pokémon!"),  "rocket_archer", _INTL("Goldenrod City"), TRQuestColor),
...
def legendaryQuestioning()
  ...
  pbMessage(_INTL("Primeiro: como é a aparência do Pokémon lendário?"))
  bodyTypes = { :BIRD => _INTL("Uma criatura voadora"), :BEAST => _INTL("Uma fera grande"), :FISH => _INTL("Uma criatura aquática"), :UNKNOWN => _INTL("Eu não sei...") }
  ...
  if chosen_bodyType == bodyTypes.length - 1
    pbCallBub(2, @event_id)
    pbMessage(_INTL("Você não sabe? Você pelo menos viu esse Pokémon?"))
    pbCallBub(2, @event_id)
    pbMessage(_INTL("Hmm... É melhor você ter mais alguma informação."))
    uncertain_answers += 1
  else
    pbCallBub(2, @event_id)
    pbMessage(_INTL("{1} que também é um Pokémon lendário? Isso parece incrível! Você tem minha atenção.",bodyTypes.values[chosen_bodyType]))
  end

  #question 2
  pbCallBub(2, @event_id)
  pbMessage(_INTL("Certo... E quanto ao tipo dele?"))
  types = { :ELECTRIC => _INTL("tipo Electric"), :FIRE => _INTL("tipo Fire"), :WATER => _INTL("tipo Water"), :ICE => _INTL("tipo Ice"), :UNKNOWN => _INTL("Eu não sei...") }
  chosen_type = optionsMenu(types.values)
  answers_so_far << types.keys[chosen_type]

  if chosen_type == types.length - 1
    pbCallBub(2, @event_id)
    pbMessage(_INTL("Então você não sabe o tipo dele... Hmm..."))
    uncertain_answers += 1
  else
    if chosen_bodyType == bodyTypes.length - 1
      pbCallBub(2, @event_id)
      pbMessage(_INTL("Hmm... Então é uma criatura desconhecida que é {1}...",types.values[chosen_type]))
    else
      pbCallBub(2, @event_id)
      pbMessage(_INTL("Hmm... {1} que é {2}.",bodyTypes.values[chosen_bodyType],types.values[chosen_type]))
    end
    susMeter = calculateSuspicionLevel(answers_so_far, uncertain_answers)
    if susMeter == 0
      pbCallBub(2, @event_id)
      pbMessage(_INTL("Isso parece bem interessante!"))
    else
      pbCallBub(2, @event_id)
      pbMessage(_INTL("Nunca ouvi falar de uma criatura assim, mas continue."))
    end
  end

  #question 3
  pbCallBub(2, @event_id)
  pbMessage(_INTL("Então... Onde esse Pokémon lendário foi avistado?"))
  locations = { :VIRIDIAN => _INTL("Perto de Viridian City"), :LAVENDER => _INTL("Perto de Lavender Town"), :CERULEAN => _INTL("Perto de Cerulean City"), :CINNABAR => _INTL("Perto de Cinnabar Island"), :UNKNOWN => _INTL("Eu não sei") }
  chosen_location = optionsMenu(locations.values)
  if chosen_location == locations.length - 1
    uncertain_answers += 1
    if uncertain_answers == 3
      pbCallBub(2, @event_id)
      pbMessage(_INTL("Você sabe alguma coisa? Isso foi uma perda de tempo enorme!"))
      return 100
    else
      pbCallBub(2, @event_id)
      pbMessage(_INTL("Como você não sabe onde ele foi avistado? Você sabe o quanto isso não me ajuda?"))
      uncertain_answers += 1
    end
  else
    ...
    if susMeter == 0
      pbCallBub(2, @event_id)
      pbMessage(_INTL("{1}, hein? Ah, sim, isso faria muito sentido... Como eu não pensei nisso antes?",locations.values[chosen_location]))
    else
      pbCallBub(2, @event_id)
      pbMessage(_INTL("Hmmm... {1}, sério? Isso me parece bem surpreendente.",locations.values[chosen_location]))
    end
  end

  #question 4
  locations_formatted = { :VIRIDIAN => _INTL("Viridian City"), :LAVENDER => _INTL("Lavender Town"), :CERULEAN => _INTL("Cerulean City"), :CINNABAR => _INTL("Cinnabar Island"), :UNKNOWN => _INTL("aquele local desconhecido") }
  pbCallBub(2, @event_id)
  pbMessage(_INTL("E em que horário do dia esse Pokémon lendário foi visto perto de {1}, exatamente?",locations_formatted.values[chosen_location]))
  time_of_day = { :DAWN => _INTL("Ao amanhecer"), :NOON => _INTL("Ao meio-dia"), :AFTERNOON => _INTL("À tarde"), :SUNSET => _INTL("Ao pôr do sol"), :NIGHT => _INTL("À noite") }
  chosen_time = optionsMenu(time_of_day.values)
  pbCallBub(2, @event_id)
  pbMessage(_INTL("Então ele foi visto perto de {1} {2}...",locations_formatted.values[chosen_location],time_of_day.values[chosen_time].downcase))
...
def sellPokemon(event_id)
  if $Trainer.party.length <= 1
    pbCallBub(2, event_id)
    pbMessage(_INTL("... Espera, eu não posso pegar seu único Pokémon!"))
    return false
  end
  pbChoosePokemon(1, 2,
                  proc { |poke|
  ...
  if chosenPokemon.personalID == exotic_pokemon_id
    pbCallBub(2, event_id)
    pbMessage(_INTL("Ah, esse é o Pokémon que você conseguiu com o colecionador, certo?"))
    pbCallBub(2, event_id)
    pbMessage(_INTL("É, não posso pegar esse. O colecionador abriu o bico para a polícia, então é arriscado demais."))
    return false
  end

  speciesName = GameData::Species.get(chosenPokemon.species).real_name
  pbCallBub(2, event_id)
  if pbConfirmMessageSerious(_INTL("Você quer me vender este {1}, é isso?",speciesName))
    pbCallBub(2, event_id)
    pbMessage(_INTL("Hmm... Vamos ver..."))
    pbWait(10)
    value = calculate_pokemon_value(chosenPokemon)
    pbCallBub(2, event_id)
    if pbConfirmMessageSerious(_INTL("\\\GPosso te dar ${1} por ele. Temos um acordo?",value.to_s))
      payout = (value * 0.7).to_i
      pbCallBub(2, event_id)
      pbMessage(_INTL("\\\GExcelente. E, claro, 30% vai para a Team Rocket. Então você fica com ${1}.",payout))
      $Trainer.money += payout
      $Trainer.remove_pokemon_at_index(pbGet(1))
      pbSEPlay("Mart buy item")
      pbCallBub(2, event_id)
      pbMessage(_INTL("\\\GPrazer em fazer negócios com você."))
      return true
    else
      pbCallBub(2, event_id)
      pbMessage(_INTL("Pare de desperdiçar meu tempo!"))
    end
  else
    pbCallBub(2, event_id)
    pbMessage(_INTL("Pare de desperdiçar meu tempo!"))
