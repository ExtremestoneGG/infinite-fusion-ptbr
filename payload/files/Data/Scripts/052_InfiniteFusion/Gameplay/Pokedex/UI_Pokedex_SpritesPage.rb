class PokemonPokedexInfo_Scene
  ...
  def showSpriteCredits(filename, generated_sprite = false)
    ...
    if !generated_sprite
      ...
      discord_name = "Autor desconhecido" if !discord_name
    else
      #todo give credits to Japeal - need to differenciate unfused sprites
      discord_name = "" #"Japeal\n(Generated)"
    end
    discord_name = "Sprite importado" if @selected_pif_sprite.local_path
  ...
  def select_sprite(brief = false)
    if @available.length > 1
      if is_main_sprite()
        if brief
          pbMessage(_INTL("Este sprite continuará sendo o sprite exibido."))
          return true
        else
          pbMessage(_INTL("Este sprite já é o sprite exibido."))
        end
      else
        message = _INTL('Gostaria de usar este sprite no lugar do sprite atual?')
      ...
      pbMessage(_INTL("Este é o único sprite disponível para este Pokémon!"))
    end
    return false
  end
