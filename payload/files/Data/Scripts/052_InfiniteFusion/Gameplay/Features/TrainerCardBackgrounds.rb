...
def purchaseCardBackground(price = 1000)
  ...
  if purchasable_cards.length <= 0
    pbMessage(_INTL("Não há mais fundos de Trainer Card disponíveis para compra!"))
    return
  end
  ...
  pbMessage(_INTL("\\GQual fundo você quer comprar?"))
  chosen = pbListScreen(_INTL("Trainer Card"), TrainerCardBackgroundLister.new(purchasable_cards))
  echoln chosen
  if chosen != nil
    name = getDisplayedName(chosen)
    if pbConfirmMessage(_INTL("\\GComprar o fundo de Trainer Card \\C[1]{1}\\C[0] por ${2}?", name, price.to_s))
      if $Trainer.money < price
        pbSEPlay("GUI sel buzzer", 80)
        pbMessage(_INTL("\\G\\C[2]Dinheiro insuficiente"))
        return false
      end
      ...
      pbSEPlay("Item get")
      pbMessage(_INTL("\\GVocê comprou o fundo de Trainer Card {1}!", name))
      if pbConfirmMessage(_INTL("Quer trocar seu Trainer Card atual pelo recém-comprado?"))
        pbSEPlay("GUI trainer card open")
        $Trainer.card_background = chosen
      else
        pbMessage(_INTL("Você pode trocar o fundo a qualquer momento ao visualizar seu Trainer Card."))
      end
      echoln $Trainer.unlocked_card_backgrounds
      return true
    end
