...
![Release](https://img.shields.io/badge/release-v1.1.5-blue)
...
These numbers describe the extracted and packaged PT-BR scope for v1.1.5. Hidden hardcoded text may still appear during public testing and should be reported with screenshots. Pokedex species/fusion descriptions are not counted as complete in this release; they are planned as future work.
...
| Submitted v1.1.5 battle text issues | 18 / 18 known strings | 0 from the current report set | 100% |
| Pokedex UI labels | Included in UI/script scope | Species/fusion entries are future work | 85% |
| Pokedex species/fusion descriptions | Deferred | Future project, not counted in v1.1.5 | 0% |
...
- A text patch for outfit descriptions. Pokedex species/fusion mappings are excluded from the v1.1.5 installer payload and planned as future work.
...
3. Download `InfiniteFusionPTBRInstaller-v1.1.5.exe`, the recommended graphical installer. Do not use this project as a game download.
4. Run `InfiniteFusionPTBRInstaller-v1.1.5.exe`.
...
If you prefer the script-based method, download the separate `PTBR-Translation-BAT-v1.1.5.zip` package and follow [docs/BAT_INSTALLER.md](docs/BAT_INSTALLER.md).

### How The Installer Works

The recommended installer is `InfiniteFusionPTBRInstaller-v1.1.5.exe`. It is a small WPF app with the PT-BR payload embedded inside the executable, so the main download does not need a folder full of files.

The `.bat` installer is not included inside the recommended `.exe` ZIP. It is shipped separately as `PTBR-Translation-BAT-v1.1.5.zip`, which includes `installer/Install-PTBR.bat`, `installer/Install-PTBR.ps1`, and `installer/Restore-Latest-Backup.bat`.
...
- Applies text-only patches to outfit descriptions. Pokedex species/fusion mappings are not included in the v1.1.5 installer payload.
- Keeps the game's executable, sprites, music, saves, and unrelated files untouched.

### Restore Tutorial

1. Open `InfiniteFusionPTBRInstaller-v1.1.5.exe`.
...
Release 1.1.5 was checked with a final validation sweep:
...
v1.1.5 does not edit saves during install/update. During graphical uninstall, it creates a save backup in `PTBR_BACKUPS/save_backups` and resets the saved language to English to avoid the known post-uninstall language crash.
...
Estes números descrevem o escopo PT-BR extraído e empacotado na v1.1.5. Textos escondidos em scripts/eventos ainda podem aparecer durante os testes públicos e devem ser reportados com prints. As descrições de espécies/fusões da Pokédex não contam como concluídas nesta versão; elas ficam para um projeto futuro.
...
| Problemas de texto de batalha reportados na v1.1.5 | 18 / 18 textos conhecidos | 0 do conjunto atual de reports | 100% |
| Labels da interface da Pokédex | Incluído no escopo de interface/scripts | Entradas de espécies/fusões ficam para depois | 85% |
| Descrições de espécies/fusões da Pokédex | Adiado | Projeto futuro, fora da contagem da v1.1.5 | 0% |
...
- Um patch de texto para descrições de roupas. As descrições da Pokédex não contam como concluídas na v1.1.5.
...
3. Baixe `InfiniteFusionPTBRInstaller-v1.1.5.exe`, o instalador gráfico recomendado. Não use este projeto como download do jogo.
4. Execute `InfiniteFusionPTBRInstaller-v1.1.5.exe`.
...
Se você atualizar Pokemon Infinite Fusion pelo updater oficial `.bat`, rode `InfiniteFusionPTBRInstaller-v1.1.5.exe` de novo **antes de abrir seu save** e clique em `Instalar / Atualizar PT-BR`. Updates oficiais podem trocar os scripts traduzidos pelos arquivos originais em inglês, então reinstalar a tradução mantém os arquivos do jogo e o save no mesmo estado esperado.
...
Se você preferir o método por script, baixe o pacote separado `PTBR-Translation-BAT-v1.1.5.zip` e siga [docs/BAT_INSTALLER.md](docs/BAT_INSTALLER.md).

### Como O Instalador Funciona

O instalador recomendado é `InfiniteFusionPTBRInstaller-v1.1.5.exe`. Ele é um pequeno app WPF com o pacote PT-BR embutido dentro do executável, então o download principal não precisa vir como uma pasta cheia de arquivos.

O instalador `.bat` não fica dentro do download principal em `.exe`. Ele é distribuído separadamente como `PTBR-Translation-BAT-v1.1.5.zip`, que inclui `installer/Install-PTBR.bat`, `installer/Install-PTBR.ps1` e `installer/Restore-Latest-Backup.bat`.
...
1. Abra `InfiniteFusionPTBRInstaller-v1.1.5.exe`.
...
A versão 1.1.5 passou por uma varredura final:
...
- Descrições de espécies/fusões da Pokédex ficam para um projeto futuro e não contam como concluídas na v1.1.5.
...
A v1.1.5 não edita saves durante instalação/atualização. Durante a desinstalação pelo `.exe`, ela cria backup dos saves em `PTBR_BACKUPS/save_backups` e volta o idioma salvo para inglês para evitar o crash conhecido após remover a tradução.
